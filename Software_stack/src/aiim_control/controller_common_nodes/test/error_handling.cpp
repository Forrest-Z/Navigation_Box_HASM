...
public:
  ErrorControllerNode()
      : ControllerBaseNode{"test_error_controller_node",
                           "",
                           cmd_topic,
                           state_topic,
                           tf_topic,
...
TEST_F(error_handling, basic) {
  ...
  const auto pub = std::make_shared<
      ...
      "test_error_controller_pub", traj_topic, state_topic, tf_topic,
      std::vector<TrajectoryProfile>{
          TrajectoryProfile{state1, 10 * ms100, ms100, ms100, "odom",
                            "base_link"},
          TrajectoryProfile{state1, 10 * ms100, ms100, ms100, "map",
                            "base_link"}});
  // ErrorListener node
  ...
  EXPECT_TRUE(ctrl->traj_error());
  {
    ...
    // ^^ On bad trajectory case, there's transforms to support the new state
    // messages So it doesn't even get executed
  }
}
