...
using aiim::control::controller_common_nodes::Diagnostic;
using aiim::control::controller_common_nodes::State;
...
using aiim::motion::motion_testing_nodes::TrajectoryProfile;
...
public:
  ...
  void match() const {
    (void)aiim::motion::motion_testing_nodes::wait_for_matched(*m_traj_sub, 1U);
    (void)aiim::motion::motion_testing_nodes::wait_for_matched(*m_state_sub,
                                                               1U);
    (void)aiim::motion::motion_testing_nodes::wait_for_matched(*m_tf_sub, 1U);
    (void)aiim::motion::motion_testing_nodes::wait_for_matched(*m_cmd_sub, 1U);
    (void)aiim::motion::motion_testing_nodes::wait_for_matched(*m_diag_sub, 1U);
  }
...
public:
  TestControllerNode()
      : ControllerBaseNode{"test_controller_base_node",
                           "",
                           cmd_topic,
                           state_topic,
                           tf_topic,
...
TEST_F(sanity_check, basic) {
  ...
  const auto state1 = make_state(0.0F, 0.0F, -1.0F, 0.0F, 1.0F, 0.0F,
                                 std::chrono::system_clock::now());
  // Publisher Node
  const auto ms100 = std::chrono::milliseconds(100LL);
  const auto pub = std::make_shared<
      aiim::motion::motion_testing_nodes::MotionTestingPublisher>(
      "test_motion_testing_node", traj_topic, state_topic, tf_topic,
      std::vector<TrajectoryProfile>{TrajectoryProfile{
          state1, 10 * ms100, ms100, ms100, "odom", "base_link"}});
  // Listener node
  ...
  // Spin
  {
    ...
    while (!pub->done() || !got_enough_commands || !got_enough_tfs ||
           !got_enough_states) {
      ...
      // got_enough_trajectories = sub->trajectories().size() + TOLI >
      // total_msgs;
    }
  ...
  // Cpmmand, and tf should have consistent values, state should be zero
  {
    ...
    for (auto idx = 1U; idx < cmds.size(); ++idx) {
      ...
      EXPECT_LE(cmd_curr.front_wheel_angle_rad, cmd_prev.front_wheel_angle_rad)
          << idx; // y
      EXPECT_LT(
          fabsf(cmd_curr.rear_wheel_angle_rad - state1.state.heading.real),
          TOL); // real
    ...
    for (auto idx = 1U; idx < tfs.size(); ++idx) {
      ...
      EXPECT_LT(fabs(tf_curr.transform.rotation.w - state1.state.heading.real),
                TOLD);
      EXPECT_LT(fabs(tf_curr.transform.rotation.z - state1.state.heading.imag),
                TOLD);
      const auto &tf_prev = tfs[idx - 1U];
      EXPECT_GE(tf_curr.transform.translation.x,
                tf_prev.transform.translation.x)
          << idx;
      EXPECT_LE(tf_curr.transform.translation.y,
  ...
  }
  {
    ...
    EXPECT_LE(std::abs(static_cast<int>(states.size()) -
