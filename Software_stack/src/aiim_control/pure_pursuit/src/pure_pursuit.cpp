...
constexpr uint32_t CAPACITY = aiim_autoware_msgs::msg::Trajectory::CAPACITY;
////////////////////////////////////////////////////////////////////////////////
PurePursuit::PurePursuit(const Config &cfg)
    : ControllerBase{controller_common::BehaviorConfig{
          3.0F, std::chrono::milliseconds{100LL},
          controller_common::ControlReference::SPATIAL}},
      m_lookahead_distance(0.0F), m_target_point{}, m_command{}, m_config(cfg),
      m_is_traj_update(false), m_reference_idx(0U), m_iterations(0U) {}

////////////////////////////////////////////////////////////////////////////////
VehicleControlCommand
PurePursuit::compute_command_impl(const TrajectoryPointStamped &current_pose) {
  ...
  if (m_config.get_is_delay_compensation()) {
    current_point = predict(
        current_point,
        start - common::time_utils::from_message(current_pose.header.stamp));
  }
  ...
  if (is_success) {
    ...
    // the following code could be used to set a hard limit on control signals regardless of the user input
    ...
    m_command.front_wheel_angle_rad = min(maxAngleRad, max(minAngleRad,m_command.front_wheel_angle_rad));
    m_command.long_accel_mps2 = min(maxAccel,max(minAccel,m_command.long_accel_mps2));
...
void PurePursuit::compute_errors(const TrajectoryPoint &current_point) {
  ...
  if (traj.points.empty()) {
    ...
    // This is the first point on trajectory just after the current_point;
    // closest is either this index, or the one just before
    ...
    if (0U != first_idx_after_current) {
      ...
      const auto dist_next =
          compute_points_distance_squared(current_point, next_point);
      const auto last_idx = first_idx_after_current - 1U;
      const auto &last_point = traj.points[last_idx];
      const auto dist_last =
          compute_points_distance_squared(current_point, last_point);
    ...
    if (dist_nearest_second < 0.1F) {
      ...
      // linear interpolation: Compute the interpolated point `P` between `A`
      // and `B`. A is the nearest point, B is the second nearest point, and C
      ...
      const float32_t rate_a =
          0.5F * (((dist_nearest_second + dist_nearest) - dist_second_nearest) /
                  dist_nearest_second);
      target = motion::motion_common::interpolate(nearest_point, second_point,
                                                  rate_a);
    }
  }
}
...
void PurePursuit::compute_lookahead_distance(const float32_t current_velocity) {
  ...
  m_lookahead_distance = std::max(
      m_config.get_minimum_lookahead_distance(),
      std::min(lookahead_distance, m_config.get_maximum_lookahead_distance()));
}
////////////////////////////////////////////////////////////////////////////////
bool8_t
PurePursuit::in_traveling_direction(const TrajectoryPoint &current_point,
                                    const TrajectoryPoint &point) const {
  bool8_t in_traveling_direction;
  const float32_t x_offset =
      compute_relative_xy_offset(current_point, point).first;
  ...
  if (((current_point.longitudinal_velocity_mps > speed_thres) &&
       is_frontward) ||
      ((current_point.longitudinal_velocity_mps < speed_thres) &&
       (!is_frontward))) { // uncrustify
    // if the current velocity is over 0, the current target should be in the
    // frontward if the current velocity is under 0 (reversed), the current
    // target should be in the backward
...
void PurePursuit::compute_interpolate_target_point(
    const TrajectoryPoint &current_point, const TrajectoryPoint &target_point,
    const uint32_t idx) {
  ...
  // compute the linear equation (ax + by + c = 0) from the prev and current
  // points link:
  ...
  const float32_t c =
      -(diff_y * prev_target_rel_x) + (diff_x * prev_target_rel_y);
  // Using the distance between the point and line, and the circule radius,
  ...
  if ((aa_bb < epsilon) || ((cc - (rr * aa_bb)) > epsilon)) {
    ...
    if (fabsf(cc - (rr * aa_bb)) < epsilon) {
      ...
      // the predicted point should be between current and its previous target
      // points
      ...
      const float32_t dist_pred_current_target =
          sqrtf((diff_pred_curr_x * diff_pred_curr_x) +
                (diff_pred_curr_y * diff_pred_curr_y));
    ...
    const float32_t rate_a = sqrtf(
        compute_points_distance_squared(m_target_point, prev_target_point) /
        aa_bb);
    m_target_point = motion::motion_common::interpolate(prev_target_point,
                                                        target_point, rate_a);
  }
}
////////////////////////////////////////////////////////////////////////////////
bool8_t
PurePursuit::compute_target_point(const TrajectoryPoint &current_point) {
  ...
  for (; idx < traj.points.size(); ++idx) {
    ...
    // judge wheter the target point is in the forward of the traveling
    // direction
    if (in_traveling_direction(current_point, target_point)) {
      ...
      if ((sqrtf(compute_points_distance_squared(
               current_point, target_point)) >= m_lookahead_distance)) {
  ...
  if (idx == traj.points.size()) {
    if (is_travel_direct) {
      ...
      // If the trajectory was not updated and there is no point in the
      // traveling direction
      is_success = false;
    }
  }
...
////////////////////////////////////////////////////////////////////////////////
float32_t
PurePursuit::compute_points_distance_squared(const TrajectoryPoint &point1,
...
////////////////////////////////////////////////////////////////////////////////
std::pair<float32_t, float32_t>
PurePursuit::compute_relative_xy_offset(const TrajectoryPoint &current,
                                        const TrajectoryPoint &target) const {
  const float32_t diff_x = target.x - current.x;
  const float32_t diff_y = target.y - current.y;
  const auto cos_pose = (current.heading.real + current.heading.imag) *
                        (current.heading.real - current.heading.imag);
...
////////////////////////////////////////////////////////////////////////////////
float32_t
PurePursuit::compute_steering_rad(const TrajectoryPoint &current_point) {
  ...
  const float32_t denominator =
      compute_points_distance_squared(current_point, m_target_point);
  const float32_t numerator =
      compute_relative_xy_offset(current_point, m_target_point).second;
  constexpr float32_t epsilon = 0.0001F;
  // equivalent to (2 * y) / (distance * distance) = (2 * sin(th)) / distance
  const float32_t carvature =
      (denominator > epsilon) ? ((2.0F * numerator) / denominator) : 0.0F;
  const float32_t steering_angle_rad =
      atanf(carvature * m_config.get_distance_front_rear_wheel());
  return steering_angle_rad;
}
////////////////////////////////////////////////////////////////////////////////
float32_t
PurePursuit::compute_command_accel_mps(const TrajectoryPoint &current_point,
  ...
  if (is_emergency) {
    ...
    const float32_t diff =
        sqrtf(compute_points_distance_squared(current_point, m_target_point));
    constexpr float32_t diff_thres =
        0.01F; // meter: To avoid dividing 0 and strong vibration
    distance = std::max(diff, diff_thres);
    target_velocity = m_target_point.longitudinal_velocity_mps;
  }
  // v^2 - v0^2 = 2ax
  const float32_t abs_accel = fabsf((target_velocity * target_velocity) -
                                    (current_point.longitudinal_velocity_mps *
                                     current_point.longitudinal_velocity_mps)) /
                              (2.0F * distance);
