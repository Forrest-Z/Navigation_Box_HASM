...
////////////////////////////////////////////////////////////////////////////////
BehaviorConfig::BehaviorConfig(const Real safe_deceleration_rate_mps2,
    ...
    : m_safe_deceleration_rate_mps2{safe_deceleration_rate_mps2},
      m_time_step{time_step}, m_is_spatial_reference{
                                  ControlReference::SPATIAL == type} {
...
void ControllerBase::set_trajectory_impl() {
  m_latest_reference =
      common::time_utils::from_message(m_reference_trajectory.header.stamp);
  if (!m_reference_trajectory.points.empty()) {
    m_latest_reference += common::time_utils::from_message(
        m_reference_trajectory.points.back().time_from_start);
  }
...
void ControllerBase::set_trajectory(const Trajectory &trajectory) {
  ...
  if (trajectory.points.empty()) {
    throw std::domain_error{
        "ControllerBase: Zero length trajectory is not expected"};
  }
  // Return value elision should hit for at most a single copy (a warning told
  // me not to move)
  m_reference_trajectory = handle_new_trajectory(trajectory);
  // This happens _after_ because of dependence on state; handle_new_trajectory
  // might mutate trajectory
  set_trajectory_impl();
}
...
Command ControllerBase::compute_command(const State &state) {
  ...
  if (state.header.frame_id != m_reference_trajectory.header.frame_id) {
    throw std::domain_error{
        "Vehicle state is not in same frame as reference trajectory"};
  }
...
////////////////////////////////////////////////////////////////////////////////
const Trajectory &
ControllerBase::handle_new_trajectory(const Trajectory &trajectory) {
  return trajectory;
}
...
Index ControllerBase::get_current_state_spatial_index() const {
  if (m_reference_trajectory.points.empty()) {
    throw std::domain_error{
        "Cannot get reference state index: no or zero length trajectory"};
  }
...
bool ControllerBase::is_state_ok(const State &state) const noexcept {
  ...
  const auto t = common::time_utils::from_message(state.header.stamp);
...
void ControllerBase::update_reference_indices(const State &new_state) noexcept {
  ...
  const auto space_f = [](const State &state, const Point &pt1,
                          const Point &pt2, const Trajectory &) -> bool {
    return motion::motion_common::is_past_point(state.state, pt1, pt2);
  };
  using motion::motion_common::update_reference_index;
  m_reference_spatial_index = update_reference_index(
      m_reference_trajectory, new_state, m_reference_spatial_index, space_f);
  // Update temporal index
  const auto time_f = [](const State &state, const Point &, const Point &pt,
                         const Trajectory &traj) -> bool {
  ...
  m_reference_temporal_index = update_reference_index(
      m_reference_trajectory, new_state, m_reference_temporal_index, time_f);
}
...
bool ControllerBase::is_past_trajectory(const State &state) const noexcept {
  ...
  // If the current reference point is not the last one, then I can't be past
  // the end
  ...
  // ^ back() shouldn't throw because the empty check happens before this
  // function
  return motion::motion_common::is_past_point(state.state, last_pt);
}

////////////////////////////////////////////////////////////////////////////////
Command
ControllerBase::compute_stop_command(const State &state) const noexcept {
  ...
  ret.long_accel_mps2 =
      state.state.longitudinal_velocity_mps >= 0.0F ? -decel : decel;
...
Point ControllerBase::predict(const Point &point,
  ...
  auto ret = point; // to fill the gaps of the model
  {
    ret.time_from_start = common::time_utils::to_message(
        common::time_utils::from_message(point.time_from_start) + dt);
    ret.x = state.at<X>();
    ret.y = state.at<Y>();
    ret.longitudinal_velocity_mps = state.at<XY_VELOCITY>();
    ret.acceleration_mps2 = state.at<XY_ACCELERATION>();
    ret.heading = motion::motion_common::from_angle(state.at<YAW>());
...
////////////////////////////////////////////////////////////////////////////////
Index ControllerBase::get_compute_iterations() const { return Index{}; }

////////////////////////////////////////////////////////////////////////////////
void compute_diagnostic(const ControllerBase &ctrl, const State &state,
                        bool use_temporal_reference, Diagnostic &out) {
  Index ref_idx = {};
  try {
  ...
  out.diag_header.iterations =
      static_cast<decltype(out.diag_header.iterations)>(
          ctrl.get_compute_iterations());
  const auto &ref = ctrl.get_reference_trajectory().points[ref_idx];
  motion::motion_common::error(state.state, ref, out);
}
