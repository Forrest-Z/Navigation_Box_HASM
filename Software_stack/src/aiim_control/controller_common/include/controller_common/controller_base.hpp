...
using motion::motion_common::Diagnostic;
using motion::motion_common::Heading;
using motion::motion_common::Index;
...
public:
  /// Positive rate of deceleration and characteristic time step of overall
  /// system
  BehaviorConfig(Real safe_deceleration_rate_mps2,
                 std::chrono::nanoseconds time_step, ControlReference type);
...
public:
  ...
  /// Main API: Given current state (and reference trajectory), compute next
  /// command. Updates the current reference point of the trajectory \throw
  /// std::domain_error If state is not in the same frame as reference
  /// trajectory
  ...
  /// \throw std::runtime_error if an empty trajectory or no trajectory is
  /// provided
...
protected:
  ...
  /// Update internal bookkeeping or the algorithm-specific representation of
  /// the trajectory associated with updating to a new trajectory, return
  /// preferred form of trajectory
...
private:
  ...
  CONTROLLER_COMMON_LOCAL void
  update_reference_indices(const State &new_state) noexcept;
  /// Determines if a given state is past the reference trajectory
  CONTROLLER_COMMON_LOCAL bool
  is_past_trajectory(const State &state) const noexcept;
  /// Just some cleanup
