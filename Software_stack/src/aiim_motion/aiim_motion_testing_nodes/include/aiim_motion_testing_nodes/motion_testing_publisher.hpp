...
public:
  MotionTestingPublisher(const std::string &node_name,
  ...
  /// Blocks until all match conditions are satisfied, then published initial
  /// trajectory (semihack)
  void match(const std::size_t tf_subs = 1U, const std::size_t state_subs = 1U,
...
private:
  ...
  min_period(const TrajectoryProfilesVec &profiles) const noexcept;
  MOTION_TESTING_NODES_LOCAL
  std::size_t end_index(const TrajectoryProfile &prof,
                        const Trajectory &traj) const noexcept;
  MOTION_TESTING_NODES_LOCAL void
  add_state_and_tf(const Header &header, const TrajectoryProfile &prof,
