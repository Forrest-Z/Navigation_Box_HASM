...
#include <aiim_motion_testing_nodes/visibility_control.hpp>
...
wait_for_matched(const std::chrono::nanoseconds timeout,
...
/// Block thread until publisher has matched with at least a certain number of
/// participants \param[in] pub The publisher to check for number of matching
/// participants \param[in] minimum_match_count Minimum number of matching
/// participants to wait for \param[in] timeout Maximum time to block for
...
MOTION_TESTING_NODES_PUBLIC std::chrono::nanoseconds wait_for_matched(
    const rclcpp::PublisherBase &pub, const std::size_t minimum_match_count,
    const std::chrono::nanoseconds timeout = std::chrono::seconds{5LL},
    const std::chrono::nanoseconds poll_period = std::chrono::milliseconds{
        1LL});

/// Block thread until subscription has matched with at least a certain number
/// of participants \param[in] sub The subscription to check for number of
/// matching participants \param[in] minimum_match_count Minimum number of
/// matching participants to wait for \param[in] timeout Maximum time to block
/// for \param[in] poll_period Time to sleep between checks \throw
/// std::runtime_error on timeout \return Time remaining before timeout, for
/// chaining convenience
MOTION_TESTING_NODES_PUBLIC std::chrono::nanoseconds wait_for_matched(
    const rclcpp::SubscriptionBase &sub, const std::size_t minimum_match_count,
    const std::chrono::nanoseconds timeout = std::chrono::seconds{5LL},
    const std::chrono::nanoseconds poll_period = std::chrono::milliseconds{
        1LL});
