...
MotionTestingPublisher::MotionTestingPublisher(
    const std::string &node_name, const std::string &traj_topic,
    const std::string &state_topic, const std::string &tf_topic,
  ...
  for (const auto &prof : profiles) {
    if (prof.state_update_period != prof.trajectory_sample_period) {
      throw std::domain_error{
          "Update period != trajectory sample period is not yet supported"};
    }
    ...
    auto s = motion_testing::make_state(x0, y0, motion_common::to_angle(yaw0),
                                        v0, prof_s0.state.acceleration_mps2,
                                        prof_s0.state.heading_rate_rps,
                                        common::time_utils::from_message(t0));
    // Trajectory
...
void MotionTestingPublisher::match(const std::size_t tf_subs,
  ...
  // WARNING Dumb hack: publish trajectory early
  {
    ...
    auto tf = m_tfs.front();
    tf.transforms[0].header.stamp = common::time_utils::to_message(
        common::time_utils::from_message(tf.transforms[0].header.stamp) -
...
MotionTestingPublisher::min_period(const TrajectoryProfilesVec &profiles) const
...
////////////////////////////////////////////////////////////////////////////////
std::size_t MotionTestingPublisher::end_index(const TrajectoryProfile &prof,
                                              const Trajectory &traj) const

    noexcept {
  auto end_idx = 0U;
  for (auto idx = 0U; idx < traj.points.

                            size();

       ++idx) {
    if (common::time_utils::from_message(traj.points[idx].time_from_start) >=
        prof.active_time) {
      end_idx = idx;
      break;
    }
...
////////////////////////////////////////////////////////////////////////////////
void MotionTestingPublisher::add_state_and_tf(const Header &header,
  ...
  tf.header.stamp = common::time_utils::to_message(
      common::time_utils::from_message(header.stamp) +
      common::time_utils::from_message(pt.time_from_start) +
  ...
  s.state = pt;
  s.header.stamp = common::time_utils::to_message(
      common::time_utils::from_message(header.stamp) +
      common::time_utils::from_message(pt.time_from_start));
...
////////////////////////////////////////////////////////////////////////////////
bool MotionTestingPublisher::done() const
