...
////////////////////////////////////////////////////////////////////////////////
bool is_past_point(const Point &state, const Point &current_pt,
...
////////////////////////////////////////////////////////////////////////////////
bool is_past_point(const Point &state, const Point &pt, const Real nx,
                   const Real ny) noexcept {
  const auto dx = (state.x - pt.x);
  const auto dy = (state.y - pt.y);

  // Check if state is past last_pt when projected onto the ray defined by
  // heading
...
bool heading_ok(const Trajectory &traj) {
  ...
  const auto bad_it =
      std::find_if(traj.points.begin(), traj.points.end(), bad_heading);
...
Real to_angle(Heading heading) noexcept {
  const auto mag2 =
      (heading.real * heading.real) + (heading.imag * heading.imag);
...
Heading nlerp(Heading a, Heading b, Real t) {
  ...
  // Normalize
  const auto s =
      1.0F / std::sqrt((ret.real * ret.real) + (ret.imag * ret.imag));
  ret.real *= s;
  ret.imag *= s;
  return ret;
}
...
void error(const Point &state, const Point &ref, Diagnostic &out) noexcept {
  ...
  out.velocity_error_mps =
      state.longitudinal_velocity_mps - ref.longitudinal_velocity_mps;
