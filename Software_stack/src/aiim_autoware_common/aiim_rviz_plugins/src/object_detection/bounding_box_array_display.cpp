...
BoundingBoxArrayDisplay::BoundingBoxArrayDisplay()
    ...
    pedestrian_color_property_ = new rviz_common::properties::ColorProperty(
            "Pedestrian Color",
            QColor(0, 0, 255.0),
            "Color to draw pedestrian boundingboxes.",
            this,
            SLOT(updateProperty()));
    cyclist_color_property_ = new rviz_common::properties::ColorProperty(
            "Cyclist Color",
            QColor(255.0, 165.0, 0),
            "Color to draw cyclist boundingboxes.",
            this,
            SLOT(updateProperty()));
    ...
    alpha_property_ = new rviz_common::properties::FloatProperty(
            "Alpha", 0.7, "Amount of transparency to apply to the boundingbox.", this, SLOT(updateProperty()));
...
void BoundingBoxArrayDisplay::processMessage(BoundingBoxArray::ConstSharedPtr msg) {
    msg_cache = msg;
...
visualization_msgs::msg::Marker::SharedPtr BoundingBoxArrayDisplay::get_marker(const BoundingBox& box) const {
...
void BoundingBoxArrayDisplay::update(float32_t wall_dt, float32_t ros_dt) { m_marker_common->update(wall_dt, ros_dt); }
...
PLUGINLIB_EXPORT_CLASS(aiim::rviz_plugins::BoundingBoxArrayDisplay, rviz_common::Display)
