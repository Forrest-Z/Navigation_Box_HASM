...
#include "aiim_geometry/bounding_box/rotating_calipers.hpp"

using aiim::common::geometry::point_adapter::x_;
using aiim::common::geometry::point_adapter::xr_;
using aiim::common::geometry::point_adapter::y_;
using aiim::common::geometry::point_adapter::yr_;
...
protected:
    ...
    void minimum_area_bounding_box() {
        ...
        box = aiim::common::geometry::bounding_box::minimum_area_bounding_box(points);
        // apex_test_tools::memory_test::stop();
    }

    void minimum_perimeter_bounding_box() {
        // apex_test_tools::memory_test::start();
        box = aiim::common::geometry::bounding_box::minimum_perimeter_bounding_box(points);
        // apex_test_tools::memory_test::stop();
    }
    template <typename IT>
    void eigenbox(const IT begin, const IT end) {
        // apex_test_tools::memory_test::start();
        box = aiim::common::geometry::bounding_box::eigenbox_2d(begin, end);
        // apex_test_tools::memory_test::stop();
    }
    template <typename IT>
    void lfit_bounding_box_2d(const IT begin, const IT end) {
        // apex_test_tools::memory_test::start();
        box = aiim::common::geometry::bounding_box::lfit_bounding_box_2d(begin, end);
        // apex_test_tools::memory_test::stop();
    }
...
using PointTypesBoundingBox = ::testing::Types<geometry_msgs::msg::Point32, aiim::common::types::PointXYZIF>;
...
TYPED_TEST(BoxTest, point_segment_distance) {
    using aiim::common::geometry::closest_segment_point_2d;
    using aiim::common::geometry::point_line_segment_distance_2d;
    // normal case
...
TYPED_TEST(BoxTest, closest_point_on_line) {
    using aiim::common::geometry::closest_line_point_2d;
    // normal case
    TypeParam p = this->make(-1.0F, 0.0F);
    TypeParam q = this->make(-1.0F, 2.0F);
    TypeParam r = this->make(1.0F, 1.0F);
...
TYPED_TEST(BoxTest, basic) {
    const std::vector<TypeParam> data{{this->make(0, 0), this->make(1, 0), this->make(1, 1), this->make(0, 1)}};
...
TYPED_TEST(BoxTest, hull) {
    ...
    for (uint32_t idx = 0U; idx < FUZZ_SIZE; ++idx) {
        const float th = ((idx * aiim::common::types::TAU) / FUZZ_SIZE) + dth;
...
TYPED_TEST(BoxTest, collinear) {
    ...
    this->test_corners({this->make(-2, -2), this->make(-2, -4), this->make(-4, -4), this->make(-4, -2)});
    this->test_orientation(0.0F);
}
...
TYPED_TEST(BoxTest, line1) {
    ...
    this->test_corners({this->make(-4, 3), this->make(-30, 27), this->make(-4, 3), this->make(-36, 27)});
...
TYPED_TEST(BoxTest, subopt_init) {
    this->points.insert(
            this->points.begin(), {this->make(8, 15), this->make(17, 0), this->make(0, 0), this->make(25, 15)});
    ...
    this->test_corners(
            {this->make(0, 0), this->make(25, 15), this->make(11.7647F, 22.0588F), this->make(13.2353F, -7.05882F)},
            0.1F);
}
...
TYPED_TEST(BoxTest, centered) {
    this->points.insert(
            this->points.begin(), {this->make(-1, 0), this->make(1, 0), this->make(0, -1), this->make(0, 1)});
...
TYPED_TEST(BoxTest, perimeter) {
    ...
    this->test_corners({this->make(3, 0), this->make(0, 4), this->make(-1.92, 2.56), this->make(1.08, -1.44)});
    ...
    this->test_corners({this->make(3, -0.1), this->make(0, 4), this->make(3, 4), this->make(0, -0.1)});
}
...
TYPED_TEST(BoxTest, eigenbox1) {
    ...
    const std::vector<TypeParam> diag_corners{this->make(4, 0), this->make(0, 4), this->make(-2, 2), this->make(2, -2)};
    this->test_corners(diag_corners);
    this->test_orientation(45.0F, 0.001F);

    //// perimeter should also produce diagonal ////
    this->minimum_perimeter_bounding_box();
    this->check(1.0F, 1.0F, r / sqrtf(2.0F), sqrtf(2.0F) * r, r * (sqrtf(2.0F) + (1.0F / sqrtf(2.0F))));
    this->test_corners(diag_corners);
    this->test_orientation(45.0F, 0.001F);
    ...
    this->test_corners({this->make(4, -1), this->make(-1, 4), this->make(4, 4), this->make(-1, -1)}, 0.25F);
...
TYPED_TEST(BoxTest, eigenbox2) {
    ...
    const std::vector<TypeParam> diag_corners{this->make(4, 4), this->make(0, 4), this->make(0, -4), this->make(4, -4)};
    ...
    this->lfit_bounding_box_2d(v.begin(), v.end());
    this->test_corners({this->make(3.5, 4.5), this->make(-1, 0), this->make(3.5, -4.5), this->make(8, 0)}, 0.2F);
...
TYPED_TEST(BoxTest, eigenbox3) {
    ...
    std::vector<TypeParam> v{
            ...
            this->make(-4, 0.04)};
    this->lfit_bounding_box_2d(v.begin(), v.end());
    this->test_corners({this->make(-4, -0.04), this->make(-4, 0.04), this->make(4, -0.04), this->make(4, 0.04)}, 0.2F);
...
TYPED_TEST(BoxTest, eig_underflow) {
    using aiim::common::geometry::bounding_box::details::Covariance2d;
