...
#include <utility>
#include <vector>
#include "aiim_geometry/convex_hull.hpp"
#include "aiim_geometry/hull_pockets.hpp"
...
typename std::vector<std::vector<typename std::iterator_traits<Iter>::value_type>> compute_hull_and_pockets(
        const Iter polygon_start, const Iter polygon_end) {
    auto convex_hull_list = std::list<typename std::iterator_traits<Iter>::value_type>{polygon_start, polygon_end};
    const auto cvx_hull_end = aiim::common::geometry::convex_hull(convex_hull_list);

    const typename decltype(convex_hull_list)::const_iterator list_beginning = convex_hull_list.begin();
    const auto pockets = aiim::common::geometry::hull_pockets(polygon_start, polygon_end, list_beginning, cvx_hull_end);
...
TYPED_TEST(TypedHullPocketsTest, triangle) {
    const auto polygon =
            std::vector<decltype(this->make(0, 0, 0))>{this->make(0, 0, 0), this->make(2, 0, 0), this->make(1, 1, 0)};
