...
protected:
    ...
    typename std::list<PointT>::const_iterator convex_hull() {
        const auto ret = aiim::common::geometry::convex_hull<PointT>(list);
        return ret;
    }
    ...
    void check_hull(
        ...
        for (auto& pt : expect) {
            ...
            while (it != last) {
                ...
                if (fabsf(pt.x - it->x) <= TOL && fabsf(pt.y - it->y) <= TOL &&
...
TYPED_TEST(TypedConvexHullTest, quadhull) {
    std::vector<TypeParam> data(
            {this->make(1, 1, 1), this->make(5, 1, 2), this->make(2, 6, 3), this->make(3, 3, 4), this->make(6, 5, 5)});
