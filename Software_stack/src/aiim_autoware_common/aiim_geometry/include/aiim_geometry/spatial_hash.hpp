...
#include <aiim_geometry/spatial_hash_config.hpp>
...
private:
    ...
    GEOMETRY_LOCAL IT insert_impl(const PointT& pt) {
        // Compute bin
        const Index idx = m_config.bin(point_adapter::x_(pt), point_adapter::y_(pt), point_adapter::z_(pt));
        // Insert into bin
...
public:
    ...
    ///         point
    const OutputVector& near(const float32_t x, const float32_t y) { return this->near_impl(x, y, 0.0F); }
    ...
    ///         all points within the configured radius, and the actual distance to the reference
    ///         point
    const OutputVector& near(const PointT& pt) { return near(point_adapter::x_(pt), point_adapter::y_(pt)); }
};
