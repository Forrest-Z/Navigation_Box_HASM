...
inline T intersection_2d(const T& pt, const T& u, const T& q, const T& v) {
    ...
    if (fabsf(den) < FEPS) {
        if (fabsf(num) < FEPS) {
            ...
            throw std::runtime_error("intersection_2d: no unique solution (either collinear or parallel)");
        }
    }
...
inline T closest_line_point_2d(const T& p, const T& q, const T& r) {
    ...
    if (len2 > std::numeric_limits<float32_t>::epsilon()) {
        ...
        throw std::runtime_error("closet_line_point_2d: line ill-defined because given points coincide");
    }
    return ret;
}
