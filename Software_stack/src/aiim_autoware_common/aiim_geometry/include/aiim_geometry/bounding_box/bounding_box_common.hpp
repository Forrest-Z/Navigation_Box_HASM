...
void compute_corners(
        decltype(BoundingBox::corners)& corners, const Point4<IT>& supports, const Point4<PointT>& directions) {
    for (uint32_t idx = 0U; idx < corners.size(); ++idx) {
        const uint32_t jdx = (idx != 3U) ? (idx + 1U) : 0U;
        const auto pt = intersection_2d(*supports[idx], directions[idx], *supports[jdx], directions[jdx]);
