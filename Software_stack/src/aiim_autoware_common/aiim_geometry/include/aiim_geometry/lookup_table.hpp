...
public:
    ...
    /// \throw std::domain_error If range is not the same size as domain
    /// \throw std::domain_error If domain is not sorted
    LookupTable1D(const std::vector<T>& domain, const std::vector<T>& range) : m_domain{domain}, m_range{range} {
    ...
    /// \throw std::domain_error If range is not the same size as domain
    /// \throw std::domain_error If domain is not sorted
    LookupTable1D(std::vector<T>&& domain, std::vector<T>&& range) : m_domain{domain}, m_range{range} {
        check_table_lookup_invariants(m_domain, m_range);
    }
