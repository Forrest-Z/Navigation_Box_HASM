...
#include <aiim_helper_functions/template_utils.hpp>
...
using expression_valid_with_return = ::aiim::common::helper_functions::expression_valid_with_return<Expression, Ts...>;
template <template <typename> class Expression, typename... Ts>
using expression_valid = ::aiim::common::helper_functions::expression_valid<Expression, Ts...>;
...
using call_bar_expression =
        decltype(std::declval<FooT>().bar(std::declval<In1>(), std::declval<const In2&>(), std::declval<In3*>()));

// Another correct way to call Foo::bar(...) since a temporary can bind to the const lvalue
// reference
template <typename FooT, typename In1, typename In2, typename In3>
using call_bar_expression2 =
...
using false_bar_expression2 = decltype(std::declval<FooT>().bar(std::declval<In1>(), std::declval<const In2&>()));

// Signature mismatch:
template <typename FooT, typename In1, typename In2, typename In3>
using false_bar_expression3 =
        decltype(std::declval<FooT>().asdasd(std::declval<In1>(), std::declval<const In2&>(), std::declval<In3*>()));
