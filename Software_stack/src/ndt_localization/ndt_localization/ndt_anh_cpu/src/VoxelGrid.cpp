#include "ndt_anh_cpu/VoxelGrid.h"
#include <inttypes.h>
#include <math.h>
#include <limits>
...
#include "ndt_anh_cpu/SymmetricEigenSolver.h"
...
template <typename PointSourceType>
int VoxelGrid<PointSourceType>::voxelId(
        PointSourceType p,
        float voxel_x,
        float voxel_y,
        float voxel_z,
...
template <typename PointSourceType>
int VoxelGrid<PointSourceType>::voxelId(
        int idx, int idy, int idz, int min_b_x, int min_b_y, int min_b_z, int size_x, int size_y, int size_z) {
        ...
        for (int idy = real_min_by_; idy <= real_max_by_; idy++)
            for (int idz = real_min_bz_; idz <= real_max_bz_; idz++) {
                ...
                if (ipoint_num >= min_points_per_voxel_) {
                    covariance = ((*tmp_cov_)[i] - 2.0 * (pt_sum * (*centroid_)[i].transpose())) / point_num +
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::radiusSearch(
        PointSourceType p, float radius, std::vector<int>& voxel_ids, int max_nn) {
    float t_x = p.x;
    float t_y = p.y;
    float t_z = p.z;
    ...
    for (int idx = min_id_x; idx <= max_id_x && nn < max_nn; idx++) {
        for (int idy = min_id_y; idy <= max_id_y && nn < max_nn; idy++) {
            for (int idz = min_id_z; idz <= max_id_z && nn < max_nn; idz++) {
                int vid = voxelId(idx, idy, idz, min_b_x_, min_b_y_, min_b_z_, vgrid_x_, vgrid_y_, vgrid_z_);
...
int VoxelGrid<PointSourceType>::nearestVoxel(
        PointSourceType query_point, Eigen::Matrix<float, 6, 1> boundaries, float max_range) {
    ...
    for (int i = lower_x; i <= upper_x; i++) {
        for (int j = lower_y; j <= upper_y; j++) {
            for (int k = lower_z; k <= upper_z; k++) {
                ...
                if ((*points_id_)[vid].size() > 0) {
                    double cur_dist =
                            sqrt((qx - c(0)) * (qx - c(0)) + (qy - c(1)) * (qy - c(1)) + (qz - c(2)) * (qz - c(2)));
...
template <typename PointSourceType>
void VoxelGrid<PointSourceType>::updateBoundaries(
        float max_x, float max_y, float max_z, float min_x, float min_y, float min_z) {
    ...
    if (new_max_x > max_x_ || new_max_y > max_y_ || new_max_z > max_z_ || new_min_x < min_x_ || new_min_y < min_y_ ||
        new_min_z < min_z_) {
        ...
        if (max_b_x > max_b_x_ || max_b_y > max_b_y_ || max_b_z > max_b_z_ || min_b_x < min_b_x_ ||
            min_b_y < min_b_y_ || min_b_z < min_b_z_) {
            ...
            for (int idx = real_min_bx_; idx <= real_max_bx_; idx++) {
                for (int idy = real_min_by_; idy <= real_max_by_; idy++) {
                    for (int idz = real_min_bz_; idz <= real_max_bz_; idz++) {
                        old_id = voxelId(idx, idy, idz, min_b_x_, min_b_y_, min_b_z_, vgrid_x_, vgrid_y_, vgrid_z_);
                        new_id = voxelId(idx, idy, idz, min_b_x, min_b_y, min_b_z, vgrid_x, vgrid_y, vgrid_z);
...
void VoxelGrid<PointSourceType>::updateVoxelContent(typename pcl::PointCloud<PointSourceType>::Ptr new_cloud) {
    ...
    for (int i = 0; i < new_cloud->points.size(); i++) {
        ...
        if (ipoint_num >= min_points_per_voxel_) {
            Eigen::Matrix3d covariance;
            covariance = ((*tmp_cov_)[vid] - 2.0 * (pt_sum * (*centroid_)[vid].transpose())) / point_num +
