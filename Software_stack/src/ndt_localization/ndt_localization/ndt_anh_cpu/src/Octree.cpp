...
void Octree<PointSourceType>::setInput(
        std::vector<Eigen::Vector3i> occupied_voxels, typename pcl::PointCloud<PointSourceType>::Ptr point_cloud) {
    octree_.reset();
    reserved_size_.reset();
    dimension_.reset();
    occupancy_check_.reset();
    ...
    std::vector<unsigned int> occupancy0(
            static_cast<int>((node_number + sizeof(unsigned int) * 8 - 1) / (sizeof(unsigned int) * 8)), 0);
...
void Octree<PointSourceType>::update(
        std::vector<Eigen::Vector3i> new_voxels, typename pcl::PointCloud<PointSourceType>::Ptr new_cloud) {
...
void Octree<PointSourceType>::updateBoundaries(std::vector<Eigen::Vector3i> new_voxels) {
    ...
    if (dst_bounds.lower_x < src_bounds.lower_x || dst_bounds.lower_y < src_bounds.lower_y ||
        dst_bounds.lower_z < src_bounds.lower_z || dst_bounds.upper_x > src_bounds.upper_x ||
        dst_bounds.upper_y > src_bounds.upper_y || dst_bounds.upper_z > src_bounds.upper_z) {
        ...
        std::vector<unsigned int> new_occupy0(
                (node_number + sizeof(unsigned int) * 8 - 1) / (sizeof(unsigned int) * 8), 0);
        ...
        for (int j = lower_y; j <= upper_y; j++) {
            for (int k = lower_z; k <= upper_z; k++) {
                ...
                if (isOccupied(node_id, 0)) {
                    ...
                    double cur_dist = sqrt(
                            (c(0) - q.x) * (c(0) - q.x) + (c(1) - q.y) * (c(1) - q.y) + (c(2) - q.z) * (c(2) - q.z));
...
// Check all childrens
template <typename PointSourceType>
void Octree<PointSourceType>::goDown(
        Eigen::Matrix<int, 4, 1> node, PointSourceType q, double& min_range, int& current_nn_voxel) {
...
template <typename PointSourceType>
void Octree<PointSourceType>::goUp(
        Eigen::Matrix<int, 4, 1> node, PointSourceType q, double& min_range, int& current_nn_voxel) {
