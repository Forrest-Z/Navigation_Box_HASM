...
#include <stdlib.h>
#include <iostream>
#include "ndt_anh_gpu/MatrixHost.h"
...
MatrixHost::MatrixHost() { fr_ = false; }
...
extern "C" __global__ void copyMatrixDevToDev(MatrixDevice input, MatrixDevice output) {
    ...
    if (row < rows_num && col < cols_num) output(row, col) = input(row, col);
}

bool MatrixHost::moveToGpu(MatrixDevice output) {
    if (rows_ != output.rows() || cols_ != output.cols()) return false;

    if (offset_ == output.offset()) {
        checkCudaErrors(
                cudaMemcpy(output.buffer(), buffer_, sizeof(double) * rows_ * cols_ * offset_, cudaMemcpyHostToDevice));
        return true;
    } else {
        double* tmp;
...
bool MatrixHost::moveToHost(MatrixDevice input) {
    if (rows_ != input.rows() || cols_ != input.cols()) return false;

    if (offset_ == input.offset()) {
        checkCudaErrors(
                cudaMemcpy(buffer_, input.buffer(), sizeof(double) * rows_ * cols_ * offset_, cudaMemcpyDeviceToHost));
        return true;
    } else {
        double* tmp;
...
SquareMatrixHost::SquareMatrixHost(int size) : MatrixHost(size, size) {}
