...
Matrix Matrix::operator*(const Matrix& M) {
    ...
    const Matrix& B = M;
    if (A.n != B.m) {
        cerr << "ERROR: Trying to multiply matrices of size "
             << "(" << A.m << "x" << A.n << ") and "
             << "(" << B.m << "x" << B.n << ")" << endl;
        exit(0);
    }
...
bool Matrix::solve(const Matrix& M, FLOAT eps) {
    ...
    if (A.m != A.n || A.m != B.m || A.m < 1 || B.n < 1) {
        cerr << "ERROR: Trying to eliminate matrices of size "
             << "(" << A.m << "x" << A.n << ") and "
             << "(" << B.m << "x" << B.n << ")" << endl;
        exit(0);
    }
    ...
    for (int32_t j = 0; j < m; j++) ipiv[j] = 0;
    ...
    for (int32_t i = 0; i < m; i++) {
        ...
        if (irow != icol) {
            for (int32_t l = 0; l < m; l++) SWAP(A.val[irow][l], A.val[icol][l])
            for (int32_t l = 0; l < n; l++) SWAP(B.val[irow][l], B.val[icol][l])
        }
        ...
        A.val[icol][icol] = 1.0;
        for (int32_t l = 0; l < m; l++) A.val[icol][l] *= pivinv;
        for (int32_t l = 0; l < n; l++) B.val[icol][l] *= pivinv;

        // Next, we reduce the rows except for the pivot one
        for (int32_t ll = 0; ll < m; ll++)
            if (ll != icol) {
                dum = A.val[ll][icol];
                A.val[ll][icol] = 0.0;
                for (int32_t l = 0; l < m; l++) A.val[ll][l] -= A.val[icol][l] * dum;
                for (int32_t l = 0; l < n; l++) B.val[ll][l] -= B.val[icol][l] * dum;
            }
    }
    ...
    for (int32_t l = m - 1; l >= 0; l--) {
        if (indxr[l] != indxc[l])
            for (int32_t k = 0; k < m; k++) SWAP(A.val[k][indxr[l]], A.val[k][indxc[l]])
    }
...
bool Matrix::lu(int32_t* idx, FLOAT& d) {
    ...
    d = 1.0;
    for (int32_t i = 0; i < n; i++) {  // Loop over rows to get the implicit scaling information.
        FLOAT big = 0.0;
    ...
    for (int32_t j = 0; j < n; j++) {      // This is the loop over columns of Crout’s method.
        for (int32_t i = 0; i < j; i++) {  // This is equation (2.3.12) except for i = j.
            sum = val[i][j];
            for (int32_t k = 0; k < i; k++) sum -= val[i][k] * val[k][j];
            val[i][j] = sum;
        }
        FLOAT big = 0.0;  // Initialize the search for largest pivot element.
        for (int32_t i = j; i < n; i++) {
            sum = val[i][j];
            for (int32_t k = 0; k < j; k++) sum -= val[i][k] * val[k][j];
            val[i][j] = sum;
        ...
        if (j != imax) {                       // Do we need to interchange rows?
            for (int32_t k = 0; k < n; k++) {  // Yes, do so...
                dum = val[imax][k];
                val[imax][k] = val[j][k];
                val[j][k] = dum;
            }
        ...
        if (j != n - 1) {  // Now, finally, divide by the pivot element.
            dum = 1.0 / val[j][j];
            for (int32_t i = j + 1; i < n; i++) val[i][j] *= dum;
        }
