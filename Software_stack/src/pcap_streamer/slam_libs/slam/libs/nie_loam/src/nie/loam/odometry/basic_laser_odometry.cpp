...
void BasicLaserOdometry::accumulateRotation(
    Angle cx, Angle cy, Angle cz, Angle lx, Angle ly, Angle lz, Angle& ox, Angle& oy, Angle& oz) {
    float srx =
        lx.cos() * cx.cos() * ly.sin() * cz.sin() - cx.cos() * cz.cos() * lx.sin() - lx.cos() * ly.cos() * cx.sin();
    ox = -asin(srx);
...
void BasicLaserOdometry::updateIMU(pcl::PointCloud<pcl::PointXYZI> const& imuTrans) {
...
void BasicLaserOdometry::process() {
        ...
        for (size_t iterCount = 0; iterCount < _maxIterations; iterCount++) {
            ...
            for (size_t i = 0; i < surfPointsFlatNum; i++) {
                ...
                if (_pointSearchSurfInd2[i] >= 0 && _pointSearchSurfInd3[i] >= 0) {
                    ...
                    // TODO: Why writing to a variable that's never read? Maybe it should
                    // be used afterwards?
                    pointProj = pointSel;
                    pointProj.x -= pa * pd2;
                    pointProj.y -= pb * pd2;
            ...
            for (int i = 0; i < pointSelNum; i++) {
                ...
                float arx =
                    ...
                    (s * srx * srz * pointOri.x - s * crz * srx * pointOri.y + s * crx * pointOri.z +
                     s * ty * crz * srx - s * tz * crx - s * tx * srx * srz) *
                        coeff.y +
                ...
                float arz = ((-s * cry * srz - s * crz * srx * sry) * pointOri.x +
                             ...
                             tx * (s * cry * srz + s * crz * srx * sry) - ty * (s * cry * crz - s * srx * sry * srz)) *
                                coeff.x +
                            (-s * crx * crz * pointOri.x - s * crx * srz * pointOri.y + s * ty * crx * srz +
                             s * tx * crx * crz) *
                                coeff.y +
            ...
            float deltaR =
                sqrt(pow(rad2deg(matX(0, 0)), 2) + pow(rad2deg(matX(1, 0)), 2) + pow(rad2deg(matX(2, 0)), 2));
    ...
    Vector3 v(
        _transform.pos.x() - _imuShiftFromStart.x(),
    ...
    pluginIMURotation(
        rx, ry, rz, _imuPitchStart, _imuYawStart, _imuRollStart, _imuPitchEnd, _imuYawEnd, _imuRollEnd, rx, ry, rz);
