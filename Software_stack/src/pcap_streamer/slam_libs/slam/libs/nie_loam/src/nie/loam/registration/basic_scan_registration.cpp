...
void BasicScanRegistration::processScanlines(
    const nie::Timestamp_ns& scanTime, std::vector<pcl::PointCloud<pcl::PointXYZI>> const& laserCloudScans) {
...
void BasicScanRegistration::reset(const nie::Timestamp_ns& scanTime) {
    _scanTime = scanTime;
...
void BasicScanRegistration::updateIMUData(Vector3& acc, IMUState& newState) {
    if (_imuHistory.size() > 0) {
        ...
        double timeDiff = nie::RepresentDurationAsDouble(newState.stamp - prevState.stamp);
...
void BasicScanRegistration::setIMUTransformFor(const float& relTime) {
    ...
    float relSweepTime = nie::RepresentDurationAsDouble(_scanTime - _sweepStart) + relTime;
...
void BasicScanRegistration::interpolateIMUStateFor(const float& relTime, IMUState& outputState) {
    double timeDiff = nie::RepresentDurationAsDouble(_scanTime - _imuHistory[_imuIdx].stamp) + relTime;
    while (_imuIdx < _imuHistory.size() - 1 && timeDiff > 0) {
        _imuIdx++;
        timeDiff = nie::RepresentDurationAsDouble(_scanTime - _imuHistory[_imuIdx].stamp) + relTime;
    }
    ...
    if (_imuIdx == 0 || timeDiff > 0) {
        ...
        double ratio =
            -timeDiff / nie::RepresentDurationAsDouble(_imuHistory[_imuIdx].stamp - _imuHistory[_imuIdx - 1].stamp);
...
void BasicScanRegistration::extractFeatures(const uint16_t& beginIdx) {
    ...
    for (size_t i = beginIdx; i < nScans; i++) {
        ...
        for (int j = 0; j < _config.nFeatureRegions; j++) {
            ...
            for (size_t k = 0; k < regionSize && smallestPickedNum < _config.maxSurfaceFlat; k++) {
                size_t idx = _regionSortIndices[k];
                size_t scanIdx = idx - scanStartIdx;
                size_t regionIdx = idx - sp;
...
void BasicScanRegistration::updateIMUTransform() {
    ...
    Vector3 imuShiftFromStart = _imuCur.position - _imuStart.position;
...
void BasicScanRegistration::setRegionBuffersFor(const size_t& startIdx, const size_t& endIdx) {
    ...
    float pointWeight = -2.0f * _config.curvatureRegion;
        ...
        if (diffNext > 0.1) {
            ...
            if (depth1 > depth2) {
                ...
                if (weighted_distance < 0.1) {
                    std::fill_n(
                        &_scanNeighborPicked[i - startIdx - _config.curvatureRegion], _config.curvatureRegion + 1, 1);
