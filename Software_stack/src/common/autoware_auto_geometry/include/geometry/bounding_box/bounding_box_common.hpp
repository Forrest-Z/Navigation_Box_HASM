...
#include <aiim_autoware_msgs/msg/detected_object.hpp>
#include <aiim_autoware_msgs/msg/shape.hpp>
...
void compute_height(const IT begin, const IT end, aiim_autoware_msgs::msg::Shape & shape)
{
  float32_t max_z = -std::numeric_limits<float32_t>::max();
  float32_t min_z = std::numeric_limits<float32_t>::max();
  for (auto it = begin; it != end; ++it) {
...
aiim_autoware_msgs::msg::Shape GEOMETRY_PUBLIC make_shape(const BoundingBox & box);
...
aiim_autoware_msgs::msg::DetectedObject GEOMETRY_PUBLIC make_detected_object(
