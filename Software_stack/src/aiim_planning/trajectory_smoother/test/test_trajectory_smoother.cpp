...
using aiim::motion::motion_testing::constant_acceleration_trajectory;
using aiim::motion::motion_testing::constant_velocity_trajectory;
using Trajectory = aiim_autoware_msgs::msg::Trajectory;
using TrajectorySmoother = aiim::planning::trajectory_smoother::TrajectorySmoother;
using TrajectorySmootherConfig = aiim::planning::trajectory_smoother::TrajectorySmootherConfig;
...
TEST(trajectory_smoother, acceleration) {
    const std::chrono::milliseconds dt(DT_MS);
    auto trajectory =
            constant_acceleration_trajectory(0, 0, 1, 10, 1, std::chrono::duration_cast<std::chrono::nanoseconds>(dt));
    trajectory.points.resize(100);
    // Zero out last point
    trajectory.points.back().longitudinal_velocity_mps = 0.0F;
...
TEST(trajectory_smoother, acceleration_deceleration) {
    ...
    const std::chrono::milliseconds dt(DT_MS);
    auto trajectory =
            constant_acceleration_trajectory(0, 0, 1, 10, 3, std::chrono::duration_cast<std::chrono::nanoseconds>(dt));
    trajectory.points.resize(p0);
    auto trajectory_decel =
            constant_acceleration_trajectory(0, 0, 1, 20, -3, std::chrono::duration_cast<std::chrono::nanoseconds>(dt));
