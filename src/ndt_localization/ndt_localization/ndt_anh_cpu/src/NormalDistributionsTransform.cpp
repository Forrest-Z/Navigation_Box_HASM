...
#include <cmath>
#include <iostream>
#include "ndt_anh_cpu/debug.h"

#define V2_ 1

namespace nie::ndt::cpu {
...
template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::auxilaryFunction_PsiMT(
        double a, double f_a, double f_0, double g_0, double mu) {
    return (f_a - f_0 - mu * g_0 * a);
}

template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::auxilaryFunction_dPsiMT(
        double g_a, double g_0, double mu) {
    return (g_a - mu * g_0);
}

template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::setInputTarget(
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::computeTransformation(
...
template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::computeDerivatives(
    ...
    for (int idx = 0; idx < source_cloud_->points.size(); idx++) {
        ...
        for (int i = 0; i < neighbor_ids.size(); i++) {
            ...
            score += updateDerivatives(
                    score_gradient, hessian, point_gradient, point_hessian, x_trans, c_inv, compute_hessian);
        }
    }
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::computePointDerivatives(
        Eigen::Vector3d& x,
        Eigen::Matrix<double, 3, 6>& point_gradient,
        Eigen::Matrix<double, 18, 6>& point_hessian,
        bool compute_hessian) {
...
template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::updateDerivatives(
        Eigen::Matrix<double, 6, 1>& score_gradient,
        Eigen::Matrix<double, 6, 6>& hessian,
        Eigen::Matrix<double, 3, 6> point_gradient,
        Eigen::Matrix<double, 18, 6> point_hessian,
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (compute_hessian) {
            for (int j = 0; j < hessian.cols(); j++) {
                hessian(i, j) +=
                        e_x_cov_x * (-gauss_d2_ * x_trans.dot(cov_dxd_pi) * x_trans.dot(c_inv * point_gradient.col(j)) +
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::computeAngleDerivatives(
...
// Copied from ndt.hpp
template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::trialValueSelectionMT(
...
// Copied from ndt.hpp
template <typename PointSourceType, typename PointTargetType>
double NormalDistributionsTransform<PointSourceType, PointTargetType>::updateIntervalMT(
        double& a_l,
        double& f_l,
        double& g_l,
        double& a_u,
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::updateHessian(
        Eigen::Matrix<double, 6, 6>& hessian,
        Eigen::Matrix<double, 3, 6> point_gradient,
        Eigen::Matrix<double, 18, 6> point_hessian,
        Eigen::Vector3d& x_trans,
    ...
    for (int i = 0; i < 6; i++) {
        ...
        for (int j = 0; j < hessian.cols(); j++) {
            hessian(i, j) +=
                    e_x_cov_x *
                    (-gauss_d2_ * x_trans.dot(cov_dxd_pi) * x_trans.dot(c_inv * point_gradient.col(j)) +
                     x_trans.dot(c_inv * point_hessian.block<3, 1>(3 * i, j)) + point_gradient.col(j).dot(cov_dxd_pi));
        }
    }
}

template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::computeHessian(
        Eigen::Matrix<double, 6, 6>& hessian,
        typename pcl::PointCloud<PointSourceType>& trans_cloud,
...
template <typename PointSourceType, typename PointTargetType>
void NormalDistributionsTransform<PointSourceType, PointTargetType>::updateVoxelGrid(
