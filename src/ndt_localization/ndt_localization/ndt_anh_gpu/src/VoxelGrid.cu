...
#include <limits>
#include "ndt_anh_gpu/VoxelGrid.h"
#include "ndt_anh_gpu/common.h"
#include "ndt_anh_gpu/debug.h"
...
#include "ndt_anh_gpu/SymmetricEigenSolver.h"
...
int GVoxelGrid::getVoxelNum() const { return voxel_num_; }

float GVoxelGrid::getMaxX() const { return max_x_; }
float GVoxelGrid::getMaxY() const { return max_y_; }
float GVoxelGrid::getMaxZ() const { return max_z_; }

float GVoxelGrid::getMinX() const { return min_x_; }
float GVoxelGrid::getMinY() const { return min_y_; }
float GVoxelGrid::getMinZ() const { return min_z_; }

float GVoxelGrid::getVoxelX() const { return voxel_x_; }
float GVoxelGrid::getVoxelY() const { return voxel_y_; }
float GVoxelGrid::getVoxelZ() const { return voxel_z_; }

int GVoxelGrid::getMaxBX() const { return max_b_x_; }
int GVoxelGrid::getMaxBY() const { return max_b_y_; }
int GVoxelGrid::getMaxBZ() const { return max_b_z_; }

int GVoxelGrid::getMinBX() const { return min_b_x_; }
int GVoxelGrid::getMinBY() const { return min_b_y_; }
int GVoxelGrid::getMinBZ() const { return min_b_z_; }

int GVoxelGrid::getVgridX() const { return vgrid_x_; }
int GVoxelGrid::getVgridY() const { return vgrid_y_; }
int GVoxelGrid::getVgridZ() const { return vgrid_z_; }
...
double* GVoxelGrid::getCentroidList() const { return centroid_; }

double* GVoxelGrid::getCovarianceList() const { return covariance_; }

double* GVoxelGrid::getInverseCovarianceList() const { return inverse_covariance_; }

int* GVoxelGrid::getPointsPerVoxelList() const { return points_per_voxel_; }
...
extern "C" __global__ void updateCovarianceS0(
        int* points_per_voxel, int voxel_num, double* eigenvalues, double* eigenvectors, int min_points_per_voxel) {
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
...
extern "C" __global__ void collectValidVoxels(
        int* valid_voxels_mark, int* candidate_voxel_id, int* output, int* writing_location, int candidate_voxel_num) {
...
extern "C" __global__ void collectValidVoxelCount(
        int* input_valid_voxel_count, int* output_valid_voxel_count, int* writing_location, int points_num) {
...
void GVoxelGrid::radiusSearch(
    ...
    checkCudaErrors(
            cudaMemcpy(valid_points_location, valid_points_mark, sizeof(int) * points_num, cudaMemcpyDeviceToDevice));
    ...
    checkCudaErrors(cudaMemcpy(
            valid_voxel_location, valid_voxel_mark, sizeof(int) * total_candidate_voxel_num, cudaMemcpyDeviceToDevice));
    ...
    collectValidVoxels<<<grid_x, block_x>>>(
            valid_voxel_mark, candidate_voxel_id, *valid_voxel_id, valid_voxel_location, total_candidate_voxel_num);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
...
extern "C" __global__ void buildParent(
        double* child_centroids,
        int* points_per_child,
        int child_grid_x,
        int child_grid_y,
        int child_grid_z,
        int child_num,
    ...
    // Moved here from the innermost for-loop above to remove a halting scenario
    // that only manifested on Volta architecture GPUs. Likely due to changes in
    // the implicit thread/warp sychronization behavior in Volta. Also, that was
    // just a bad place for a sync barrier (in a nested for loop, inside an if).
    __syncthreads();
}
...
extern "C" __global__ void insertPointsToGrid(
    ...
    for (int i = index; i < points_num; i += stride) {
        ...
        float t_z = z[i];
        int voxel_id =
                voxelId(t_x, t_y, t_z, voxel_x, voxel_y, voxel_z, min_b_x, min_b_y, min_b_z, vgrid_x, vgrid_y, vgrid_z);
...
extern "C" __global__ void scatterPointsToVoxels(
    ...
    for (int i = idx; i < points_num; i += stride) {
        int voxel_id = voxelId(
                x[i], y[i], z[i], voxel_x, voxel_y, voxel_z, min_b_x, min_b_y, min_b_z, vgrid_x, vgrid_y, vgrid_z);
...
void GVoxelGrid::nearestNeighborSearch(
        float* trans_x,
        float* trans_y,
        float* trans_z,
        int point_num,
