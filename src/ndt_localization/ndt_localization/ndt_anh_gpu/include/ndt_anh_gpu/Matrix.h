...
CUDAH void Matrix::setCellVal(int row, int col, double val) { buffer_[(row * cols_ + col) * offset_] = val; }
...
CUDAH double* Matrix::cellAddr(int row, int col) {
    if (row >= rows_ || col >= cols_ || row < 0 || col < 0) return NULL;
...
CUDAH double& Matrix::operator()(int row, int col) { return buffer_[(row * cols_ + col) * offset_]; }

CUDAH void Matrix::set(int row, int col, double val) { buffer_[(row * cols_ + col) * offset_] = val; }

CUDAH double& Matrix::operator()(int index) { return buffer_[index * offset_]; }

CUDAH double Matrix::at(int row, int col) const { return buffer_[(row * cols_ + col) * offset_]; }
...
CUDAH bool Matrix::transpose(Matrix& output) {
    if (rows_ != output.cols_ || cols_ != output.rows_) return false;
...
CUDAH bool Matrix::inverse(Matrix& output) {
    if (rows_ != cols_ || rows_ == 0 || cols_ == 0) return false;
...
CUDAH Matrix Matrix::col(int index) { return Matrix(rows_, 1, offset_ * cols_, buffer_ + index * offset_); }

CUDAH Matrix Matrix::row(int index) { return Matrix(1, cols_, offset_, buffer_ + index * cols_ * offset_); }
