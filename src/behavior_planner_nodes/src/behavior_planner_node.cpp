...
void BehaviorPlannerNode::init()
{
  ...
  if (declare_parameter("enable_object_collision_estimator").get<bool>()) {
    ...
    m_modify_trajectory_aiim_client = this->create_client<aiim_autoware_msgs::srv::ModifyTrajectory>("/estimate_collision");
    while (!m_modify_trajectory_aiim_client->wait_for_service(1s)) {
      if (!rclcpp::ok()) {
  ...
  m_ego_state_sub = this->create_subscription<State>(
    "/vehicle/vehicle_kinematic_state", QoS{10},
  ...
  // Setup publishers
  m_trajectory_pub =
    this->create_publisher<Trajectory>("trajectoryOriginal", QoS{10});
  m_trajectory_aiim_pub =
    this->create_publisher<aiim_autoware_msgs::msg::Trajectory>("/modified_trajectory", QoS{10});  
...
aiim_autoware_msgs::msg::Trajectory BehaviorPlannerNode::getTrajectory(const Trajectory &trajectory) const
{
  aiim_autoware_msgs::msg::Trajectory aiimTrajectory;
  aiim_autoware_msgs::msg::TrajectoryPoint bufferPoint;
  aiimTrajectory.set__header(trajectory.header);
  for (long unsigned int i = 0; i < trajectory.points.size(); i++)
  {
    double w, x, y, z;
    // double roll, pitch, sinr_cosp, cosr_cosp, sinp;
    double yaw, siny_cosp, cosy_cosp;
    w = trajectory.points[i].pose.orientation.w;
    x = trajectory.points[i].pose.orientation.x;
    y = trajectory.points[i].pose.orientation.y;
    z = trajectory.points[i].pose.orientation.z;
    ...
    bufferPoint.set__acceleration_mps2(trajectory.points[i].acceleration_mps2);
    bufferPoint.set__front_wheel_angle_rad(trajectory.points[i].front_wheel_angle_rad);
    bufferPoint.set__heading_rate_rps(trajectory.points[i].heading_rate_rps);
    bufferPoint.set__lateral_velocity_mps(trajectory.points[i].lateral_velocity_mps);
    bufferPoint.set__longitudinal_velocity_mps(trajectory.points[i].longitudinal_velocity_mps);
    bufferPoint.set__rear_wheel_angle_rad(trajectory.points[i].rear_wheel_angle_rad);
    bufferPoint.set__time_from_start(trajectory.points[i].time_from_start);
    bufferPoint.heading.set__real(static_cast<float>(cos(yaw / 2)));
    bufferPoint.heading.set__imag(static_cast<float>(sin(yaw / 2)));
    bufferPoint.set__x(static_cast<float>(trajectory.points[i].pose.position.x));
    bufferPoint.set__y(static_cast<float>(trajectory.points[i].pose.position.y));

    aiimTrajectory.points.push_back(bufferPoint);
  }
  return aiimTrajectory;
}
...
void BehaviorPlannerNode::on_ego_state(const State::SharedPtr & msg)
{
  ...
  else
  {
    ...
    aiim_autoware_msgs::msg::Trajectory aiimTrajectory = getTrajectory(trajectory);

    if (m_modify_trajectory_aiim_client)
    {
      ...
      request->original_trajectory = aiimTrajectory;
      auto result =
          m_modify_trajectory_aiim_client->async_send_request(
              request,
              std::bind(
    ...
    {
      m_trajectory_aiim_pub->publish(aiimTrajectory);
    }
  }
}
...
void BehaviorPlannerNode::modify_trajectory_response(
  rclcpp::Client<aiim_autoware_msgs::srv::ModifyTrajectory>::SharedFuture future)
{
  ...
  // collision estimator fails or if there is obstacle on first point
    if (trajectory.points.empty()) {
        ...
        trajectory = getTrajectory(Trajectory);
    }
    // m_trajectory_pub->publish(trajectory);
    m_trajectory_aiim_pub->publish(trajectory);
}
