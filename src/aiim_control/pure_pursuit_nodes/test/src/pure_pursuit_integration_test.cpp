...
void create_traj(Trajectory &traj, uint32_t traj_size,
                 float32_t heading = PI / 4.0F, float32_t offset = 1.0F,
  ...
  for (uint32_t idx = 0U; idx < traj_size; ++idx) {
    ...
    traj.points[idx].heading = aiim::motion::motion_common::from_angle(heading);
  }
}
...
void create_current_pose(TrajectoryPointStamped &current_stamp, float32_t x,
  ...
  current_stamp.state.heading =
      aiim::motion::motion_common::from_angle(heading);
...
public:
  MotionControllerPubSub(
      const char8_t *const name, const char8_t *const pose_topic,
      const char8_t *const trajectory_topic, const char8_t *const command_topic,
      const char8_t *const diag_topic, const char8_t *const traj_frame_id,
      const char8_t *const pose_frame_id, const uint32_t num_msgs,
      const std::chrono::nanoseconds pub_period)
      : Node(name), m_pose_pub_ptr(create_publisher<TrajectoryPointStamped>(
                        pose_topic, rclcpp::QoS{10})),
        m_trajectory_pub_ptr(
            create_publisher<Trajectory>(trajectory_topic, rclcpp::QoS{10})),
        m_tf2_pub_ptr(
            create_publisher<tf2_msgs::msg::TFMessage>("tf", rclcpp::QoS{10})),
        m_pose_frame_id(pose_frame_id), m_trajectory_frame_id(traj_frame_id),
        m_last_pub(std::chrono::steady_clock::now()), m_num_msgs(num_msgs),
        m_pub_period(pub_period), m_iteration(0U), m_done(false),
        m_sub_ptr(create_subscription<VehicleControlCommand>(
        ...
        m_num_correct_output() // zero initialization
  {
    ...
    RCLCPP_INFO(get_logger(),
                "MotionControllerPubSub initialized. Now start pub task.");
    (void)diag_topic;
    m_timer =
        create_wall_timer(m_pub_period, [this]() -> void { task_function(); });
  }

  bool8_t done() {
    const bool8_t pub_done = is_done();
    const bool8_t wait_done =
        (std::chrono::steady_clock::now() - get_last_pub() >
         std::chrono::seconds(1LL));
  ...
  bool8_t success() const {
    ...
    RCLCPP_INFO(get_logger(),
                "\tShould receive: "s + std::to_string(m_num_msgs * 2U));
    RCLCPP_INFO(get_logger(),
                "\tActual received: "s + std::to_string(m_num_received_msgs));
    ...
    message_received =
        message_received && (m_num_received_msgs > (m_num_msgs * 7 / 10));
    // Definitely shouldn't get too many
    message_received =
        message_received && (m_num_received_msgs <= m_num_msgs * 2U);

    RCLCPP_INFO(get_logger(),
                "\tShould have correct answer: "s + std::to_string(m_num_msgs));
    RCLCPP_INFO(get_logger(), "\tActual have correct answer: "s +
                                  std::to_string(m_num_correct_output));
    bool8_t is_correct = m_num_correct_output <= m_num_msgs;
    is_correct =
        is_correct && ((m_num_correct_output * 2U) > (m_num_msgs * 7 / 10));
...
private:
  ...
  const std::chrono::steady_clock::time_point &get_last_pub() const {
    return m_last_pub;
  }
  ...
  void task_function() {
    ...
    if (m_iteration > total_msgs) {
      ...
      // Given the trajectory (slope = PI / 4) and various velocity, angle, and
      // position values
      if ((m_iteration % 3U) == 0U) {
        ...
        const float32_t z_float =
            static_cast<float32_t>(m_tf_msg.transform.rotation.z);
        m_tf_msg.transform.rotation.w = sqrtf(1.0F - z_float * z_float);
        m_tf_msg.header.stamp = aiim::common::time_utils::to_message(right_now);
        m_tf2_msg.transforms.push_back(m_tf_msg);
        m_tf2_pub_ptr->publish(m_tf2_msg);
        m_tf2_msg.transforms.clear();
        // traj update
        m_traj_msg.header.stamp = m_tf_msg.header.stamp;
        create_traj(m_traj_msg, 100U, PI / 4.0F,
                    static_cast<float32_t>(m_tf_msg.transform.translation.x));
        m_traj_msg.header.frame_id = m_trajectory_frame_id.c_str();
        m_trajectory_pub_ptr->publish(m_traj_msg);
      } else {
        // pose update
        const float32_t offset = static_cast<float32_t>(m_iteration % 3U - 1);
        create_current_pose(m_pose_msg, offset * 2.0F, offset * 2.0F, 0.0F,
                            offset * 10, 0.0F, 0.0F, m_pose_frame_id.c_str());
        m_pose_msg.header.stamp =
            aiim::common::time_utils::to_message(right_now);
  ...
  void update(VehicleControlCommand::SharedPtr msg) {
    ...
    if ((iteration % 3) == 2U) {
      ...
      // Poses that are on the trajectory and those velocity equals to 0 are
      // checked The first pose topic is always on the origin of the trajectory
      // by the TF transform, and the velocity is 0 (constant), and only the yaw
      // angle is changing (from 0 to PI/2). Since the angle of the trajectory
      // is invariant PI/4 throughout the test and the velocity of each
      // trajectory point N equals to the origin x value of the trajectory + K
      // (linearly increasing), the desired steering angle and acceleration can
      // be estimated.
      constexpr float32_t TOL = 1.0E-5F;
      const float32_t target_velocity =
          static_cast<float32_t>(iteration / 3U) + 6.0F;
      constexpr float32_t distance =
          5.0F * sqrtf(2.0F); // minimum distance = 6.0
      const float32_t accel =
          ((target_velocity * target_velocity) / (2.0F * distance));
      ...
      const float32_t relative_y =
          (-sinf(heading_rad) * 5.0F) + (cosf(heading_rad) * 5.0F);
      const float32_t carvature = (2.0F * relative_y) / denominator;
      const float32_t angle = atanf(carvature * 2.7F);
      if ((fabsf(accel - msg->long_accel_mps2) < TOL) &&
...
int32_t main(const int32_t argc, char8_t **const argv) {
  ...
  std::shared_ptr<MotionControllerPubSub> tester_ptr =
      std::make_shared<MotionControllerPubSub>(
          "pure_puresuit_test", pose_topic.c_str(), trajectory_topic.c_str(),
          command_topic.c_str(), diag_topic.c_str(), traj_frame_id.c_str(),
          pose_frame_id.c_str(), num_msgs, period);
