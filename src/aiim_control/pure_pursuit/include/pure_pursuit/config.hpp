...
public:
  /// \brief Constructor
  /// \param[in] minimum_lookahead_distance The minimum lookahead distance
  /// (meter)
  ///            for the pure pursuit
  /// \param[in] maximum_lookahead_distance The maximum lookahead distance
  /// (meter)
  ///            for the pure pursuit
  ...
  /// \param[in] is_interpolate_lookahead_point The boolean whether using the
  /// interpolation
  ///            for determining the target position
  /// \param[in] is_delay_compensation The boolean whethre using the delay
  /// compensation
  ///            for estimating the current vehicle position at the current
  ///            timestamp
  /// \param[in] emergency_stop_distance The emergency stop distance for the
  /// emergency stop \param[in] speed_thres_traveling_direction The speed
  /// threshold for
  ///            determining the traveling direction
  /// \param[in] distance_front_rear_wheel The distance between front and rear
  /// wheels
  Config(const float32_t minimum_lookahead_distance,
  ...
  /// \brief Gets the boolean whether using the interpolation to get the target
  /// point \return Fixed value
  bool8_t get_is_interpolate_lookahead_point() const noexcept;
  /// \brief Gets the boolean whether using the delay compensation to estimate
  /// the current point \return Fixed value
