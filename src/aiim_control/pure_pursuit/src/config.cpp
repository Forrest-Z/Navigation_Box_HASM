...
////////////////////////////////////////////////////////////////////////////////
Config::Config(const float32_t minimum_lookahead_distance,
  ...
  if (m_minimum_lookahead_distance <= 0.0F) {
    throw std::domain_error(
        "pure_pursuit::Config minimum lookahead distance is lower than 0");
  }
  if (m_maximum_lookahead_distance <= 0.0F) {
    throw std::domain_error(
        "pure_pursuit::Config maximum lookahead distance is lower than 0");
  }
  if (m_speed_to_lookahead_ratio <= 0.0F) {
    throw std::domain_error(
        "pure_pursuit::Config: speed to lookahead ratio is lower than 0");
  }
  if (emergency_stop_distance <= 0.0F) {
    throw std::domain_error(
        "pure_pursuit::Config: emergency_stop_distance is lower than 0");
  }
  if (m_speed_thres_traveling_direction < 0.0F) {
    throw std::domain_error("pure_pursuit::Config: "
                            "speed_thres_traveling_direction is lower than 0");
  }
