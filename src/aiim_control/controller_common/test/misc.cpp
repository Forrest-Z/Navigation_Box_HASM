...
using aiim::control::controller_common::Trajectory;
...
public:
  MiscTestController()
      : ControllerBase{BehaviorConfig{3.0F, milliseconds(100LL),
...
TEST_F(motion_model, constant_velocity) {
  ...
  const auto s = make_state(5.0F, 10.0F, -1.0F, 10.0F, 0.0F, 0.0F,
  ...
  // zero
  {
    ...
    EXPECT_LT(
        std::fabs(s.longitudinal_velocity_mps - p.longitudinal_velocity_mps),
        TOL);
    ...
    const auto dt =
        from_message(p.time_from_start) - from_message(s.time_from_start);
...
TEST_F(motion_model, constant_acceleration) {
  constexpr auto TOL = 1.0E-3F;
  const auto s = make_state(-5.0F, -10.0F, 1.0F, 1.0F, -1.0F, 0.0F,
                            std::chrono::system_clock::now())
                     .state;
...
TEST_F(motion_model, constant_turn_rate) {
  constexpr auto TOL = 1.0E-3F;
  const auto s = make_state(0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.1F,
                            std::chrono::system_clock::now())
                     .state;
  // apex_test_tools::memory_test::start();
  // positive
  {
    ...
    EXPECT_LT(
        std::fabs(p.longitudinal_velocity_mps - s.longitudinal_velocity_mps),
        TOL);
    EXPECT_LT(std::fabs(s.acceleration_mps2 - p.acceleration_mps2), TOL);
    EXPECT_LT(std::fabs(s.heading_rate_rps - p.heading_rate_rps), TOL);
    const auto dt =
