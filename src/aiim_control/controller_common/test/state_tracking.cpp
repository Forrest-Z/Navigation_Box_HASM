...
TEST_F(state_tracking, no_trajectory) {
  EXPECT_THROW(controller_.get_current_state_spatial_index(),
               std::domain_error);
  EXPECT_THROW(controller_.get_current_state_temporal_index(),
               std::domain_error);
...
// subsequent calls should give the index of the point the current state is just
// after So if points are at 0, 1, 2..., then 0, and 0.5 should result in
// reference indices of 0 and 1.0 and 1.5 should give 1
...
// Going past the end of the trajectory should keep you fixed at the end, even
// if you go backwards
TEST_F(state_tracking, past_end) {
  ...
  {
    auto state =
        make_state(-1.0F, 0.0F, 0.0F, 10.0F, 0.0F, 0.0F, t0 + (dt * 10U));
    state.header.frame_id = "foo";
    // apex_test_tools::memory_test::resume();
    (void)controller_.compute_command(state);
  ...
  {
    auto state =
        make_state(-9.01F, 0.0F, 0.0F, 10.0F, 0.0F, 0.0F, t0_ + (dt * 90U));
    state.header.frame_id = "foo";
    // apex_test_tools::memory_test::resume();
    (void)controller_.compute_command(state);
  ...
  {
    auto state =
        make_state(-10.0F, 0.0F, 0.0F, 10.0F, 0.0F, 0.0F, t0 + (dt * 100U));
    state.header.frame_id = "foo";
    // apex_test_tools::memory_test::resume();
    (void)controller_.compute_command(state);
  ...
  {
    auto state =
        make_state(-15.0F, 0.0F, 0.0F, 10.0F, 0.0F, 0.0F, t0 + (dt * 150U));
    state.header.frame_id = "foo";
    // apex_test_tools::memory_test::resume();
    (void)controller_.compute_command(state);
    EXPECT_EQ(controller_.get_current_state_spatial_index(), 99U);
  ...
  {
    auto state =
        make_state(-5.0F, 0.0F, 0.0F, 10.0F, 0.0F, 0.0F, t0 + (dt * 50U));
    state.header.frame_id = "foo";
    // apex_test_tools::memory_test::resume();
    (void)controller_.compute_command(state);
    EXPECT_EQ(controller_.get_current_state_spatial_index(), 99U);
