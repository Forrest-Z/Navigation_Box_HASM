...
public:
  BadControllerNode()
      : ControllerBaseNode{"bad_controller_base_node",
                           "",
                           cmd_topic,
                           state_topic,
                           tf_topic,
...
protected:
  // Ugly global because lambdas can only convert to function pointers if they
  // dont capture Also ugly memory leak to avoid -Wexit-time-destructors
  static std::stringstream *ss;

  // Override ROS 2 logging to point to some static ostream which we can then
  // read
  static void fake_log(const rcutils_log_location_t *, int, const char *,
  ...
  void SetUp() override {
    ...
    traj_pub_ =
        ctrl_->create_publisher<Trajectory>(traj_topic, rclcpp::QoS{10LL});
    sub_->match();
...
TEST_F(bad_cases, empty_trajectory_frame) {
  ...
  auto state = make_state(0.0F, 0.0F, -1.0F, 9.0F, 1.0F, 0.0F,
  ...
  // EXPECT_EQ(last_line, "try_compute: empty trajectory frame, possibly
  // uninitialized, deferring");
}
...
TEST_F(bad_cases, empty_state_frame) {
  ...
  auto state = make_state(0.0F, 0.0F, -1.0F, 0.0F, 1.0F, 0.0F,
                          std::chrono::system_clock::now());
