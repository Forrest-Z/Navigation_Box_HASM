...
void Lanelet2MapVisualizer::visualize_map_callback(
  rclcpp::Client<aiim_autoware_msgs::srv::HADMapService>::SharedFuture response)
{
...
{
  m_client =
    this->create_client<aiim_autoware_msgs::srv::HADMapService>("HAD_Map_Service");

  m_viz_pub = this->create_publisher<visualization_msgs::msg::MarkerArray>(
    "viz_had_map",
    rclcpp::QoS(rclcpp::KeepLast(5U)).transient_local());

  auto request = std::make_shared<aiim_autoware_msgs::srv::HADMapService::Request>();
  ...
  if (!use_geom_bounds) {
    request->requested_primitives.push_back(
      aiim_autoware_msgs::srv::HADMapService::Request::FULL_MAP);
  } else {
    request->requested_primitives.push_back(
      aiim_autoware_msgs::srv::HADMapService::Request::DRIVEABLE_GEOMETRY);
