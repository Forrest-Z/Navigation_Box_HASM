...
#include <aiim_geometry/visibility_control.hpp>
...
#include "aiim_helper_functions/crtp.hpp"
...
class GEOMETRY_PUBLIC Config : public aiim::common::helper_functions::crtp<Derived> {
public:
    ...
    Index bin(const float32_t x, const float32_t y, const float32_t z) const { return this->impl().bin_(x, y, z); }
...
protected:
    ...
    Index x_index(const float32_t x) const {
        return static_cast<Index>(std::floor((std::min(std::max(x, m_min_x), m_max_x) - m_min_x) * m_side_length_inv));
    }
    /// \brief Computes the index in the y basis direction
    /// \param[in] y The y value of a point
    /// \return The x offset of the index
    Index y_index(const float32_t y) const {
        return static_cast<Index>(std::floor((std::min(std::max(y, m_min_y), m_max_y) - m_min_y) * m_side_length_inv));
    }
    /// \brief Computes the index in the z basis direction
    /// \param[in] z The z value of a point
    /// \return The x offset of the index
    Index z_index(const float32_t z) const {
        return static_cast<Index>(std::floor((std::min(std::max(z, m_min_z), m_max_z) - m_min_z) * m_side_length_inv));
    }
    ...
    Index bin_impl(const float32_t x, const float32_t y) const { return bin_impl(x_index(x), y_index(y)); }
