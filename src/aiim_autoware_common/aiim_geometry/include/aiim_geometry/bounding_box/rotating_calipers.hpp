...
#include <aiim_geometry/bounding_box/bounding_box_common.hpp>
...
uint32_t update_angles(const Point4<PointT>& edges, Point4<PointT>& directions) {
    ...
    const float32_t sin_th = cross_2d(directions[advance_idx], edges[advance_idx]) / best_edge_dir_mag;
    for (uint32_t idx = 0U; idx < edges.size(); ++idx) {
...
BoundingBox minimum_area_bounding_box(const IT begin, const IT end) {
    const auto metric_fn = [](const decltype(BoundingBox::size)& pt) -> float32_t { return pt.x * pt.y; };
