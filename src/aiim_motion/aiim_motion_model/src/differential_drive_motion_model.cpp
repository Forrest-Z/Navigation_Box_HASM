...
#include <aiim_helper_functions/float_comparisons.hpp>
...
template <>
CvtrMotionModel::State::Matrix
CvtrMotionModel::crtp_jacobian(const State &state,
                               const std::chrono::nanoseconds &dt) const {
  CvtrMotionModel::State::Matrix jacobian{
      CvtrMotionModel::State::Matrix::Identity()};
  const auto t = std::chrono::duration<float32_t>{dt}.count();
  const auto theta = state.at<YAW>();
  const auto v = state.at<XY_VELOCITY>();
  const auto w = state.at<YAW_CHANGE_RATE>();
...
CatrMotionModel::crtp_predict(const CatrMotionModel::State &state,
  ...
  if (common::helper_functions::comparisons::abs_eq_zero(w, kEpsilon)) {
    ...
    new_state.at<X>() = x + inv_w * t * a * sin(t_w_plus_theta) -
                        inv_w * v * sin(theta) +
                        inv_w * v * sin(t_w_plus_theta) +
                        inv_w_2 * a * (-cos(theta) + cos(t_w_plus_theta));
    new_state.at<Y>() = y - inv_w * t * a * cos(t_w_plus_theta) +
                        inv_w * v * cos(theta) -
...
template <>
CatrMotionModel::State::Matrix
CatrMotionModel::crtp_jacobian(const State &state,
                               const std::chrono::nanoseconds &dt) const {
  CatrMotionModel::State::Matrix jacobian{
      CatrMotionModel::State::Matrix::Identity()};
  const auto t = std::chrono::duration<float32_t>{dt}.count();
  const auto theta = state.at<YAW>();
  const auto v = state.at<XY_VELOCITY>();
  const auto a = state.at<XY_ACCELERATION>();
  ...
  if (common::helper_functions::comparisons::abs_eq_zero(w, kEpsilon)) {
    ...
    jacobian(x_index, theta_index) =
        -0.5F * t * t * a * sin(theta) - t * v * sin(theta);
    jacobian(x_index, v_index) = t * cos(theta);
    jacobian(x_index, w_index) =
        -0.5F * t * t * t * a * sin(theta) - t * t * v * sin(theta);
    jacobian(x_index, a_index) = 0.5F * t * t * cos(theta);
    // Y row non-identity entries
    jacobian(y_index, theta_index) =
        0.5F * t * t * a * cos(theta) + t * v * cos(theta);
    jacobian(y_index, v_index) = t * sin(theta);
    jacobian(y_index, w_index) =
        0.5F * t * t * t * a * cos(theta) + t * t * v * cos(theta);
    ...
    jacobian(x_index, v_index) = sin_theta_diff_inv_w;
    jacobian(x_index, w_index) =
        t * t * a * inv_w * cos_next_theta + t * v * inv_w * cos_next_theta -
    ...
    jacobian(y_index, v_index) = -cos_theta_diff_inv_w;
    jacobian(y_index, w_index) =
        t * t * a * inv_w * sin_next_theta + t * v * inv_w * sin_next_theta +
