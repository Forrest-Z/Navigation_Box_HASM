...
// * Adding aiim prefix
// * Removing dependency to apex_test_tool
// * Removing dependency to osrf_testing_tools_cpp
...
using aiim::motion::motion_common::Command;
using aiim::motion::motion_common::Point;
using aiim::motion::motion_common::State;
using aiim::motion::motion_common::to_angle;
using aiim::motion::motion_common::Trajectory;
...
TEST(interpolation, clamp) {
  const auto fn_floating = [](auto val, auto min, auto max, auto res,
                              auto tol) {
    EXPECT_LT(
        std::fabs(aiim::motion::motion_common::clamp(val, min, max) - res), tol)
        << val << ", " << min << ", " << max << ", " << res << ", " << tol;
  };
...
TEST(interpolation, interpolation) {
  const auto fn_floating = [](auto a, auto b, auto t, auto res, auto tol) {
    EXPECT_LT(
        std::fabs(aiim::motion::motion_common::interpolate(a, b, t) - res), tol)
        << a << ", " << b << ", " << t << ", " << res << ", " << tol;
  };
...
TEST(interpolation, slerp_2d) {
  using aiim::motion::motion_common::Heading;
  ...
  // Plain check
  const auto test_case = [=](Heading a, Heading b, Real t, Heading res,
                             Real tol) {
    using aiim::motion::motion_common::nlerp;
    const auto ret = nlerp(a, b, t);
    EXPECT_LT(std::fabs(ret.imag - res.imag), tol)
        << ret.imag << ", " << res.imag;
    EXPECT_LT(std::fabs(ret.real - res.real), tol)
        << ret.real << ", " << res.real;
  };
  ...
  const auto test_case_dual_path = [=](Heading a, Heading b, Real t, Real tol) {
    ...
    const auto t_ = aiim::motion::motion_common::clamp(t, 0.0F, 1.0F);
    ...
    if (HasFailure()) {
      const auto ret = nlerp(a, b, t);
      std::cout << "Angles: " << th_a << ", " << th_b << "; " << th_t << ", "
                << to_angle(ret) << "\n";
    }
  };
  const auto test_suite = [=](Heading a, Heading b, Real tol, Real tol2,
                              bool b_snap = false) {
  ...
  // apex_test_tools::memory_test::stop();
  // Test case below doesn't work, but I'm pretty sure it's more because the
  // test doesn't work for crazy angles test_suite(make(-cs45, -cs45),
  // make(-cs45, cs45), TOL, TOL);
...
TEST(interpolation, angle_arithmetic) {
  ...
  const auto test_fn = [](auto a, auto b, auto res, auto tol) {
    ...
    EXPECT_LT(std::fabs(dth - res), tol)
        << a << ", " << b << ", " << res << ", " << dth;
  };
...
void generic_checks(const Point &s, const Point &p, std::chrono::nanoseconds dt,
                    float TOL) {
...
TEST(interpolation, trajectory_subsample) {
  ...
  // apex_test_tools::memory_test::start();
  aiim::motion::motion_common::sample(t_ref, result, dt);
  ...
  for (auto idx = 0U; idx < result.points.size(); ++idx) {
    ...
    EXPECT_LT(
        std::fabs(pt.longitudinal_velocity_mps - ref.longitudinal_velocity_mps),
        TOL);
