...
public:
  ...
  LimitsConfig(Extremum longitudinal_velocity_mps,
               Extremum lateral_velocity_mps, Extremum acceleration_mps2,
               Extremum yaw_rate_rps, Extremum jerk_mps3,
               Extremum steer_angle_rad, Extremum steer_angle_rate_rps);
...
public:
  VehicleConfig(Real length_cg_front_axel_m, Real length_cg_rear_axel_m,
                Real front_cornering_stiffness_N,
                Real rear_cornering_stiffness_N, Real mass_kg,
                Real inertia_kgm2, Real width_m, Real front_overhang_m,
...
/// \brief Specifies the weights used for particular state weights in the
/// least-squares objective
///        function of the mpc solver
class MOTION_COMMON_PUBLIC StateWeight {
public:
  StateWeight(Real pose, Real heading, Real longitudinal_velocity,
              Real lateral_velocity, Real yaw_rate, Real acceleration,
              Real jerk, Real steer_angle, Real steer_angle_rate);
...
/// \brief Specifies various parameters specific to the optimization problem and
/// it's behaviors
