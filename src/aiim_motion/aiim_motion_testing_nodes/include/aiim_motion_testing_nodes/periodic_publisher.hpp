...
template <typename T> class MOTION_TESTING_NODES_PUBLIC PeriodicPublisher {
public:
  PeriodicPublisher(rclcpp::Node &node, const PeriodCountTopic &cfg,
                    std::list<T> msgs = {})
      : m_msgs{msgs} {
    ...
    // Publisher
    m_pub = node.create_publisher<T>(
        cfg.topic, rclcpp::QoS{cfg.count}.transient_local().reliable());
    // Timer
    m_timer = node.create_wall_timer(cfg.period, [this]() -> void {
      if (!m_msgs.empty()) {
