...
State make_state(float x0, float y0, float heading, float v0, float a0,
  ...
  start_state.header.stamp = common::time_utils::to_message(t);
...
////////////////////////////////////////////////////////////////////////////////
Trajectory constant_trajectory(const State &start_state,
  ...
  ret.points.back().time_from_start =
      common::time_utils::to_message(std::chrono::nanoseconds::zero());
  ...
  for (auto i = 1LL; i < capacity; ++i) {
    ...
    // heading update
    const auto w =
        (last_state.heading.real * dw) - (last_state.heading.imag * dz);
    const auto z =
        (last_state.heading.real * dz) + (last_state.heading.imag * dw);
    ...
    const auto ds = dt_s * (last_state.longitudinal_velocity_mps +
                            (0.5F * dt_s * last_state.acceleration_mps2));
    next_state.x += c * ds;
    next_state.y += s * ds;

    next_state.time_from_start = common::time_utils::to_message(dt * i);

    ret.points.push_back(next_state);
  }
  ret.header.stamp =
      common::time_utils::to_message(std::chrono::system_clock::now());
  return ret;
}

////////////////////////////////////////////////////////////////////////////////
Trajectory bad_heading_trajectory(const State &start_state,
                                  const std::chrono::nanoseconds dt) {
  Trajectory ret{rosidl_runtime_cpp::MessageInitialization::ALL};
  const auto capacity = 100LL; // TEMP
  ret.points.reserve(capacity);
...
////////////////////////////////////////////////////////////////////////////////
Trajectory constant_velocity_trajectory(const float x0, const float y0,
                                        const float heading, const float v0,
                                        const std::chrono::nanoseconds dt) {
  return constant_acceleration_turn_rate_trajectory(x0, y0, heading, v0, 0.0F,
                                                    0.0F, dt);
}

////////////////////////////////////////////////////////////////////////////////
Trajectory constant_acceleration_trajectory(const float x0, const float y0,
                                            const float heading, const float v0,
                                            const float a0,
                                            const std::chrono::nanoseconds dt) {
  return constant_acceleration_turn_rate_trajectory(x0, y0, heading, v0, a0,
                                                    0.0F, dt);
}

////////////////////////////////////////////////////////////////////////////////
Trajectory constant_velocity_turn_rate_trajectory(
    const float x0, const float y0, const float heading, const float v0,
    const float turn_rate, const std::chrono::nanoseconds dt) {
  return constant_acceleration_turn_rate_trajectory(x0, y0, heading, v0, 0.0F,
                                                    turn_rate, dt);
}
...
Trajectory constant_acceleration_turn_rate_trajectory(
    const float x0, const float y0, const float heading, const float v0,
    const float a0, const float turn_rate, const std::chrono::nanoseconds dt) {
  State start_state = make_state(x0, y0, heading, v0, a0, turn_rate,
...
////////////////////////////////////////////////////////////////////////////////
void next_state(const Trajectory &trajectory, State &state, const uint32_t hint,
                Generator *const gen) {
  (void)trajectory;
  (void)state;
  (void)gen;
...
////////////////////////////////////////////////////////////////////////////////
Index progresses_towards_target(const Trajectory &trajectory,
                                const Point &target,
  ...
  for (auto idx = Index{}; idx < trajectory.points.size(); ++idx) {
    ...
    const auto dot = (pt.heading.real * target.heading.real) +
                     (pt.heading.imag * target.heading.imag);
...
Index dynamically_feasible(const Trajectory &trajectory, const Real tolerance) {
  ...
  for (auto idx = Index{1}; idx < trajectory.points.size(); ++idx) {
    const auto &pt = trajectory.points[idx];
    const auto dt_ = common::time_utils::from_message(pt.time_from_start) -
                     common::time_utils::from_message(last_pt.time_from_start);
    const auto dt =
        std::chrono::duration_cast<std::chrono::duration<Real>>(dt_).count();
    ...
    }
    {
      ...
      const auto c2 = (pt.heading.real - pt.heading.imag) *
                      (pt.heading.real + pt.heading.imag);
      const auto s2 = Real{2} * pt.heading.real * pt.heading.imag;
      const auto dx = (ds * c) - (dn * s);
      const auto dx2 = (ds * c2) - (dn * s2);
      ok = (check_fn(last_pt.x + dx, pt.x, "x") ||
            check_fn(last_pt.x + dx2, pt.x, "x2")) &&
           ok;
      const auto dy = (s * ds) + (dn * c);
      const auto dy2 = (s2 * ds) + (dn * c2);
      ok = (check_fn(last_pt.y + dy, pt.y, "y") ||
            check_fn(last_pt.y + dy2, pt.y, "y2")) &&
           ok;
    }
    if (!ok) {
      return idx;
