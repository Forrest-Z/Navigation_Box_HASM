...
TEST(constant_trajectory, stationary) {
  const auto x0 = 3.0F;
  const auto y0 = -5.0F;
  const auto traj = aiim::motion::motion_testing::constant_velocity_trajectory(
...
TEST(constant_trajectory, constant_velocity) {
  ...
  const auto traj = aiim::motion::motion_testing::constant_velocity_trajectory(
      x0, y0, heading, v0, dt);
  const auto dt_s =
      std::chrono::duration_cast<std::chrono::duration<float_t>>(dt).count();
  for (auto i = 0U; i < traj.points.size(); ++i) {
    const auto &t = traj.points[i];
...
TEST(constant_trajectory, constant_acceleration) {
  ...
  const auto dt = std::chrono::milliseconds(100LL);
  const auto traj =
      aiim::motion::motion_testing::constant_acceleration_trajectory(
