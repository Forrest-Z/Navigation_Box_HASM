...
RecordReplayPlannerNode::RecordReplayPlannerNode(const rclcpp::NodeOptions& node_options)
    ...
    m_ego_sub = create_subscription<State>(
            ego_topic, QoS{10}, [this](const State::SharedPtr msg) { on_ego(msg); }, SubAllocT{});
...
void RecordReplayPlannerNode::on_ego(const State::SharedPtr& msg) {
    ...
    if (m_planner->is_recording()) {
        ...
        if (added) {
            ...
            m_recorded_markers.markers.push_back(to_marker(
                    msg->state, msg->header.frame_id, static_cast<int>(m_planner->get_record_length()), "record"));
            m_trajectory_viz_pub->publish(m_recorded_markers);
        }
    ...
    if (m_planner->is_replaying()) {
        ...
        if (m_modify_trajectory_client) {
            ...
            auto result = m_modify_trajectory_client->async_send_request(
                    request,
                    std::bind(&RecordReplayPlannerNode::modify_trajectory_response, this, std::placeholders::_1));
        } else {
    ...
    else
    {
        ...
        TrajectoryPoint pt = msg->state;
        pt.set__longitudinal_velocity_mps(0.0);
        pt.set__acceleration_mps2(0.0);
        pt.set__front_wheel_angle_rad(0.0);
        pt.set__lateral_velocity_mps(0.0);
        pt.set__heading_rate_rps(0.0);
        pt.set__rear_wheel_angle_rad(0.0);
        stopTrajectory.points.push_back(pt);
        stopTrajectory.set__header(msg->header);
        m_trajectory_pub->publish(stopTrajectory);
    }
}

void RecordReplayPlannerNode::modify_trajectory_response(rclcpp::Client<ModifyTrajectory>::SharedFuture future) {
...
void RecordReplayPlannerNode::record_handle_accepted(const std::shared_ptr<GoalHandleRecordTrajectory> goal_handle) {
    // Store the goal handle otherwise the action gets canceled immediately
    m_recordgoalhandle = goal_handle;
    RCLCPP_INFO(this->get_logger(), "Recording trajectory to file: " + goal_handle->get_goal()->record_path);
    m_planner->start_recording(goal_handle->get_goal()->record_path);
...
void RecordReplayPlannerNode::replay_handle_accepted(const std::shared_ptr<GoalHandleReplayTrajectory> goal_handle) {
    // Store the goal handle otherwise the action gets canceled immediately
...
RCLCPP_COMPONENTS_REGISTER_NODE(aiim::planning::recordreplay_planner_nodes::RecordReplayPlannerNode)
