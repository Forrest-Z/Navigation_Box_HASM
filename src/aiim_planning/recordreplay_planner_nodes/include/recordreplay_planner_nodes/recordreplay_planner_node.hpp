...
using PlannerPtr = std::unique_ptr<aiim::planning::recordreplay_planner::RecordReplayPlanner>;
...
private:
    ...
    RECORDREPLAY_PLANNER_NODES_LOCAL Marker
    to_marker(const TrajectoryPoint& traj_point, const std::string& frame_id, int32_t index, const std::string& ns);
    ...
    RECORDREPLAY_PLANNER_NODES_LOCAL MarkerArray to_markers(const Trajectory& traj, const std::string& ns);
