...
* `aiim_autoware_msgs/msg/VehicleKinematicState` is the state used as recorded points for replaym, and also to prune starting point of replay trajectory
* `aiim_autoware_msgs/msg/BoundingBoxArray` is list of bounding boxes of obstacle, that thre reaply trajectory stops for to avoid collision.

Outputs:

* `aiim_autoware_msgs/msg/Trajectory` is the trajectory that gets published
