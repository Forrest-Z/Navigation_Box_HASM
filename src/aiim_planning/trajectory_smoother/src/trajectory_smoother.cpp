...
void TrajectorySmoother::Filter(Trajectory& trajectory) {
    if (trajectory.points.size() > 2) {
        ...
        for (std::size_t i = trajectory.points.size() - 1 - zero_run_length; i < trajectory.points.size() - 1; ++i) {
        ...
        for (std::size_t i = 1; i < trajectory.points.size() - 1; ++i) {
            ...
            for (std::size_t j = 0; j < m_kernel.size(); ++j) {
                ...
                sum += trajectory.points[static_cast<std::size_t>(points_index)].longitudinal_velocity_mps *
                       m_kernel[j];
            }
