...
#include <aiim_geometry/intersection.hpp>
...
using aiim::common::geometry::bounding_box::minimum_perimeter_bounding_box;
...
bool8_t isTooFarAway(
        const TrajectoryPoint way_point, const BoundingBox obstacle_bbox, const float32_t distance_threshold) {
...
int32_t detectCollision(
    ...
    const auto vehicle_length = vehicle_param.front_overhang() + vehicle_param.length_cg_front_axel() +
                                vehicle_param.length_cg_rear_axel() + vehicle_param.rear_overhang();
    const auto vehicle_width = vehicle_param.width();
    const auto vehicle_diagonal = sqrtf((vehicle_width * vehicle_width) + (vehicle_length * vehicle_length));
    ...
    for (std::size_t i = 0; i < trajectory.points.size(); ++i) {
        waypoint_bboxes.boxes.push_back(waypointToBox(trajectory.points[i], vehicle_param, safety_factor));
    }
    for (std::size_t i = 0; (i < trajectory.points.size()) && (collision_index == -1); ++i) {
        ...
        for (const auto& obstacle_bbox : obstacles.boxes) {
            if (!isTooFarAway(trajectory.points[i], obstacle_bbox, distance_threshold) &&
                aiim::common::geometry::intersect(
                        waypoint_bbox.corners.begin(),
                        waypoint_bbox.corners.end(),
                        obstacle_bbox.corners.begin(),
                        obstacle_bbox.corners.end())) {
...
int32_t getStopIndex(
        const Trajectory& trajectory, const int32_t collision_index, const float32_t stop_margin) noexcept {
    ...
    // If an obstacle is closer than the stop margin, the car stops 
    if (collision_index == stop_index)
    {
        stop_index = 1;
    }
...
ObjectCollisionEstimator::ObjectCollisionEstimator(
        ObjectCollisionEstimatorConfig config, TrajectorySmoother smoother) noexcept
...
void ObjectCollisionEstimator::updatePlan(Trajectory& trajectory) noexcept {
    ...
    auto collision_index = detectCollision(
            trajectory, m_obstacles, m_config.vehicle_config, m_config.safety_factor, m_trajectory_bboxes);
...
void ObjectCollisionEstimator::updateObstacles(const BoundingBoxArray& bounding_boxes) noexcept {
