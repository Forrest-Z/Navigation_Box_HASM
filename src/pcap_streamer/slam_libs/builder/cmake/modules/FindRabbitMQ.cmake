...
# From: https://fossies.org/linux/MaxScale/cmake/FindRabbitMQ.cmake
...
if(RABBITMQ_FOUND)
    ...
    check_c_source_compiles("#include <amqp.h>\n int main(){if(AMQP_DELIVERY_PERSISTENT){return 0;}return 1;}"
                            HAVE_RABBITMQ50)

    if(HAVE_RABBITMQ50)
        execute_process(
            COMMAND grep "#define *AMQP_VERSION_MAJOR" "${RABBITMQ_HEADERS}/amqp.h"
            COMMAND sed -e "s/.* //"
            OUTPUT_VARIABLE AMQP_VERSION_MAJOR
            OUTPUT_STRIP_TRAILING_WHITESPACE)
        execute_process(
            COMMAND grep "#define *AMQP_VERSION_MINOR" "${RABBITMQ_HEADERS}/amqp.h"
            COMMAND sed -e "s/.* //"
            OUTPUT_VARIABLE AMQP_VERSION_MINOR OUTPUT_STRIP_TRAILING_WHITESPACE)
        execute_process(
            COMMAND grep "#define *AMQP_VERSION_PATCH" "${RABBITMQ_HEADERS}/amqp.h"
            COMMAND sed -e "s/.* //"
            OUTPUT_VARIABLE AMQP_VERSION_PATCH OUTPUT_STRIP_TRAILING_WHITESPACE)
        ...
        message(
            WARNING
                "RabbitMQ-C library does not have AMQP_DELIVERY_PERSISTENT. Version 0.5 or newer is required but version ${AMQP_VERSION} was found."
        )
    endif()
